...
C3D_AFFINE_TOOL_METADATA = Metadata(
    id="ce718ff13f9663ad8457d8405ce364c12372f8d1",
    name="C3dAffineTool",
...
def c3d_affine_tool(
    ...
    ret = C3dAffineToolOutputs(
        ...
        itk_transform_outfile=execution.output_file(f"{out_itk_transform}", optional=True) if out_itk_transform is not None else None,
        irtk_transform_outfile=execution.output_file(f"{out_irtk_transform}", optional=True) if out_irtk_transform is not None else None,
        matrix_transform_outfile=execution.output_file(f"{out_matfile}", optional=True) if out_matfile is not None else None,
    )
    execution.run(cargs)
    return ret
