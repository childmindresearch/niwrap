...
def filmbabe(
    ...
    temporal_ar_mrf_prec: float | int | None = -1,
    temporal_ar_mrf_prec_alias: float | int | None = -1,
    ...
    cargs.extend(["--datafile", execution.input_file(datafile_alias)])
    cargs.extend(["--mask", execution.input_file(mask_alias)])
    cargs.extend(["--designfile", execution.input_file(designfile_alias_2)])
    cargs.extend(["--frf", execution.input_file(frf)])
    if verbose_flag_alias:
        cargs.append("--verbose")
    if debug_level_alias_2 is not None:
        cargs.extend(["--debuglevel", debug_level_alias_2])
    if timing_on_flag:
        cargs.append("--to")
    if help_flag_alias:
        cargs.append("--help")
    ...
    if flobs_dir is not None:
        cargs.extend(["--fd", flobs_dir])
    if prior_covar_file_alias is not None:
        cargs.extend(["--priorcovarfile", execution.input_file(prior_covar_file_alias)])
    if prior_mean_file_alias is not None:
        cargs.extend(["--priormeanfile", execution.input_file(prior_mean_file_alias)])
    if log_dir_alias_2 is not None:
        cargs.extend(["--logdir", log_dir_alias_2])
    if num_iterations is not None:
        cargs.extend(["--ni", str(num_iterations)])
    if temporal_ar_mrf_prec_alias is not None:
        cargs.extend(["--tarmrfprec", str(temporal_ar_mrf_prec_alias)])
    if temporal_ar_flag:
        cargs.append("--tarard")
    if num_trace_samples_alias is not None:
        cargs.extend(["--ntracesamps", str(num_trace_samples_alias)])
    if temporal_ar_order is not None:
        cargs.extend(["--ntar", str(temporal_ar_order)])
    ret = FilmbabeOutputs(
        root=execution.output_file("."),
        output_dir=execution.output_file(f"output"),
    )
    execution.run(cargs)
    return ret
