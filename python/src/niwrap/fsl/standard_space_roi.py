...
def standard_space_roi(
    ...
    if mask_mask is not None:
        cargs.extend(["-maskMASK", execution.input_file(mask_mask)])
    if mask_none_flag:
        cargs.append("-maskNONE")
    if roi_fov_flag:
        cargs.append("-roiFOV")
    if roi_mask is not None:
        cargs.extend(["-roiMASK", execution.input_file(roi_mask)])
    if roi_none_flag:
        cargs.append("-roiNONE")
    if ss_ref is not None:
        cargs.extend(["-ssref", execution.input_file(ss_ref)])
    if alt_input is not None:
        cargs.extend(["-altinput", execution.input_file(alt_input)])
    if debug_flag:
        cargs.append("-d")
    if bet_premask_flag:
        cargs.append("-b")
    ...
    ret = StandardSpaceRoiOutputs(
        ...
        out_image=execution.output_file(f"{outfile}"),
    )
    execution.run(cargs)
    return ret
