...
def eddy_combine(
    ...
    ret = EddyCombineOutputs(
        ...
        combined_data=execution.output_file(f"{output_path}/combined_data.nii.gz"),
        combined_bvals=execution.output_file(f"{output_path}/combined_bvals"),
        combined_bvecs=execution.output_file(f"{output_path}/combined_bvecs"),
    )
    execution.run(cargs)
    return ret
