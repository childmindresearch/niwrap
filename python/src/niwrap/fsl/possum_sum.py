...
POSSUM_SUM_METADATA = Metadata(
    id="df02755d9fffb908d0384df82b32a7f5dac53aaa",
    name="possum_sum",
...
def possum_sum(
    ...
    cargs.extend(["-o", execution.input_file(output_signal)])
    if num_processors is not None:
        cargs.extend(["-n", str(num_processors)])
    if verbose_flag:
        cargs.append("-v")
    ret = PossumSumOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{pathlib.Path(output_signal).name}"),
    )
    execution.run(cargs)
    return ret
