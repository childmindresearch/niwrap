...
FSL_REG_METADATA = Metadata(
    id="ac1e41854fe87b905970cb9e2ad35a5ef9f797fe",
    name="fsl_reg",
...
def fsl_reg(
    ...
    if flirt_options is not None:
        cargs.extend(["-flirt", flirt_options])
    if fnirt_options is not None:
        cargs.extend(["-fnirt", fnirt_options])
    ret = FslRegOutputs(
        root=execution.output_file("."),
        output_transform_file=execution.output_file(f"{output_file}"),
    )
    execution.run(cargs)
    return ret
