...
def mm(
    ...
    mrf_prec_start: float | int | None = None,
    mrf_prec_multiplier: float | int | None = None,
    init_multiplier: float | int | None = None,
    ...
    More information: https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/MM
    ...
    cargs = []
    cargs.append("mm")
    cargs.extend(["--sdf", execution.input_file(spatial_data_file)])
    cargs.extend(["-m", execution.input_file(mask_file)])
    if verbose_flag:
        cargs.append("--verbose")
    if debug_level is not None:
        cargs.extend(["--debug", debug_level])
    if timing_flag:
        cargs.append("--timingon")
    if example_epi_file is not None:
        cargs.extend(["--edf", execution.input_file(example_epi_file)])
    if log_directory is not None:
        cargs.extend(["--logdir", log_directory])
    if nonspatial_flag:
        cargs.append("--ns")
    if fix_mrf_precision_flag:
        cargs.append("--fmp")
    if mrf_prec_start is not None:
        cargs.extend(["--mps", str(mrf_prec_start)])
    if mrf_prec_multiplier is not None:
        cargs.extend(["--mpm", str(mrf_prec_multiplier)])
    if init_multiplier is not None:
        cargs.extend(["--im", str(init_multiplier)])
    if no_update_theta_flag:
        cargs.append("--nut")
    if zfstat_flag:
        cargs.append("--zfstatmode")
    if phi is not None:
        cargs.extend(["--phi", str(phi)])
    if niters is not None:
        cargs.extend(["--ni", str(niters)])
    if threshold is not None:
        cargs.extend(["--th", str(threshold)])
    ret = MmOutputs(
