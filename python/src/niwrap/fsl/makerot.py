...
def makerot(
    ...
    if axis is not None:
        cargs.extend(["--axis", axis])
    if cov is not None:
        cargs.extend(["--cov", execution.input_file(cov)])
    if center is not None:
        cargs.extend(["--centre", center])
    if output_file is not None:
        cargs.extend(["--out", execution.input_file(output_file)])
    if verbose_flag:
        cargs.append("--verbose")
    if help_flag:
        cargs.append("--help")
    cargs.extend(["--theta", str(theta)])
    ret = MakerotOutputs(
        root=execution.output_file("."),
        matrix_output=execution.output_file(f"{pathlib.Path(output_file).name}", optional=True) if output_file is not None else None,
    )
    execution.run(cargs)
    return ret
