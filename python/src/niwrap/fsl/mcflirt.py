...
def mcflirt(
    ...
    ret = McflirtOutputs(
        ...
        mean_img=execution.output_file(f"{pathlib.Path(out_file).name}_mean_reg.ext", optional=True) if out_file is not None else None,
        out_file_outfile=execution.output_file(f"{pathlib.Path(out_file).name}", optional=True) if out_file is not None else None,
        par_file=execution.output_file(f"{pathlib.Path(out_file).name}.par", optional=True) if out_file is not None else None,
        rms_files=execution.output_file(f"{pathlib.Path(out_file).name}_*.rms", optional=True) if out_file is not None else None,
        std_img=execution.output_file(f"{pathlib.Path(out_file).name}_sigma.ext", optional=True) if out_file is not None else None,
        variance_img=execution.output_file(f"{pathlib.Path(out_file).name}_variance.ext", optional=True) if out_file is not None else None,
    )
    execution.run(cargs)
    return ret
