...
def fnirtfileutils(
    ...
    warp_res: float | int | None = None,
    knot_space: float | int | None = None,
    ...
    cargs.extend(["--in", execution.input_file(input_coefs)])
    if ref_volume is not None:
        cargs.extend(["--ref", execution.input_file(ref_volume)])
    if out_field is not None:
        cargs.extend(["--out", out_field])
    if output_format is not None:
        cargs.extend(["--outformat", output_format])
    if warp_res is not None:
        cargs.extend(["--warpres", str(warp_res)])
    if knot_space is not None:
        cargs.extend(["--knotspace", str(knot_space)])
    if jacobian_output is not None:
        cargs.extend(["--jac", jacobian_output])
    if jacobian_matrix_output is not None:
        cargs.extend(["--matjac", jacobian_matrix_output])
    if with_aff:
    ...
    ret = FnirtfileutilsOutputs(
        ...
        output_field_file=execution.output_file(f"{out_field}.nii.gz", optional=True) if out_field is not None else None,
        jacobian_output_file=execution.output_file(f"{jacobian_output}.nii.gz", optional=True) if jacobian_output is not None else None,
        jacobian_matrix_file=execution.output_file(f"{jacobian_matrix_output}.nii.gz", optional=True) if jacobian_matrix_output is not None else None,
    )
    execution.run(cargs)
    return ret
