...
def pnm_evs(
    ...
    order_cardiac: float | int | None = 2,
    order_respiratory: float | int | None = 1,
    order_mult_cardiac: float | int | None = 0,
    order_mult_respiratory: float | int | None = 0,
    csf_mask: InputPathType | None = None,
    rvt_file: InputPathType | None = None,
    heartrate_file: InputPathType | None = None,
    rvt_smooth: float | int | None = 0,
    heartrate_smooth: float | int | None = None,
    ...
    """
    pnm_evs by University of Oxford (Mark Jenkinson).
    ...
    cargs.extend(["--in", execution.input_file(input_file)])
    cargs.extend(["--out", output_file])
    cargs.extend(["--tr", str(tr_value)])
    if cardiac_file is not None:
        cargs.extend(["--cardiac", execution.input_file(cardiac_file)])
    if respiratory_file is not None:
        cargs.extend(["--respiratory", execution.input_file(respiratory_file)])
    if order_cardiac is not None:
        cargs.extend(["--oc", str(order_cardiac)])
    if order_respiratory is not None:
        cargs.extend(["--or", str(order_respiratory)])
    if order_mult_cardiac is not None:
        cargs.extend(["--multc", str(order_mult_cardiac)])
    if order_mult_respiratory is not None:
        cargs.extend(["--multr", str(order_mult_respiratory)])
    if csf_mask is not None:
        cargs.extend(["--csfmask", execution.input_file(csf_mask)])
    if rvt_file is not None:
        cargs.extend(["--rvt", execution.input_file(rvt_file)])
    if heartrate_file is not None:
        cargs.extend(["--heartrate", execution.input_file(heartrate_file)])
    if rvt_smooth is not None:
        cargs.extend(["--rvtsmooth", str(rvt_smooth)])
    if heartrate_smooth is not None:
        cargs.extend(["--heartratesmooth", str(heartrate_smooth)])
    if slice_direction is not None:
        cargs.extend(["--slicedir", slice_direction])
    if slice_order is not None:
        cargs.extend(["--sliceorder", slice_order])
    if slice_timing_file is not None:
        cargs.extend(["--slicetiming", execution.input_file(slice_timing_file)])
    if debug_flag:
        cargs.append("--debug")
    if verbose_flag:
