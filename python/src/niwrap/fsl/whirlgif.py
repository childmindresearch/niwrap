...
def whirlgif(
    ...
    if loop_count is not None:
        cargs.extend(["-loop", str(loop_count)])
    if delay_time is not None:
        cargs.extend(["-time", str(delay_time)])
    if disp_flag is not None:
        cargs.extend(["-disp", disp_flag])
    if list_file is not None:
        cargs.extend(["-i", execution.input_file(list_file)])
    cargs.extend([execution.input_file(f) for f in input_files])
    ret = WhirlgifOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{pathlib.Path(outfile).name}") if outfile is not None else None,
    )
    execution.run(cargs)
    return ret
