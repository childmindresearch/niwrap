...
def fsl_deface(
    ...
    if shift_nud is not None and (len(shift_nud) != 3): 
        raise ValueError(f"Length of 'shift_nud' must be 3 but was {len(shift_nud)}")
    ...
    if defacing_mask is not None:
        cargs.extend(["-d", defacing_mask])
    if cropped_struc is not None:
        cargs.extend(["-n", cropped_struc])
    if orig_to_std_mat is not None:
        cargs.extend(["-m13", orig_to_std_mat])
    if orig_to_cropped_mat is not None:
        cargs.extend(["-m12", orig_to_cropped_mat])
    if cropped_to_std_mat is not None:
        cargs.extend(["-m23", cropped_to_std_mat])
    if shift_nud is not None:
        cargs.extend(["-nud", *map(str, shift_nud)])
    ...
    if qc_images is not None:
        cargs.extend(["-p", qc_images])
    ret = FslDefaceOutputs(
        root=execution.output_file("."),
        outfile=execution.output_file(f"{outfile}.nii.gz"),
        out_defacing_mask=execution.output_file(f"{defacing_mask}.nii.gz", optional=True) if defacing_mask is not None else None,
        out_cropped_struc=execution.output_file(f"{cropped_struc}.nii.gz", optional=True) if cropped_struc is not None else None,
        out_orig_to_std_mat=execution.output_file(f"{orig_to_std_mat}.mat", optional=True) if orig_to_std_mat is not None else None,
        out_orig_to_cropped_mat=execution.output_file(f"{orig_to_cropped_mat}.mat", optional=True) if orig_to_cropped_mat is not None else None,
        out_cropped_to_std_mat=execution.output_file(f"{cropped_to_std_mat}.mat", optional=True) if cropped_to_std_mat is not None else None,
        qc_image_original=execution.output_file(f"{qc_images}_defaced_original.png", optional=True) if qc_images is not None else None,
        qc_image_cropped=execution.output_file(f"{qc_images}_defaced_cropped.png", optional=True) if qc_images is not None else None,
    )
    execution.run(cargs)
    return ret
