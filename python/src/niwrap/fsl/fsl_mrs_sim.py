...
def fsl_mrs_sim(
    ...
    autophase_ppm: float | int | None = None,
    ...
    cargs.extend(["-m", metab])
    cargs.extend(["-b", execution.input_file(metab_file)])
    cargs.extend(["-s", execution.input_file(custom_spinsys)])
    ...
    if autophase_ppm is not None:
        cargs.extend(["-p", str(autophase_ppm)])
    if lcm_loc is not None:
        cargs.extend(["--lcmIN", lcm_loc])
    if mm_json is not None:
        cargs.extend(["--MM", execution.input_file(mm_json)])
    if output_te is not None:
        cargs.extend(["-e", str(output_te)])
    if overwrite_flag:
    ...
    ret = FslMrsSimOutputs(
        ...
        generated_output=execution.output_file(f"{output_folder}/<generated_file>") if output_folder is not None else None,
    )
    execution.run(cargs)
    return ret
