...
def qboot(
    ...
    lambda_param: float | int | None = None,
    delta_param: float | int | None = None,
    alpha_param: float | int | None = None,
    ...
    cargs.extend(["-r", execution.input_file(bvecs_file)])
    cargs.extend(["-b", execution.input_file(bvals_file)])
    if log_dir is not None:
        cargs.extend(["--ld", log_dir])
    if forcedir_flag:
        cargs.append("--forcedir")
    if q_file is not None:
        cargs.extend(["--q", execution.input_file(q_file)])
    if model_type is not None:
        cargs.extend(["--model", str(model_type)])
    if lmax_order is not None:
        cargs.extend(["--lmax", str(lmax_order)])
    if npeaks is not None:
        cargs.extend(["--npeaks", str(npeaks)])
    if threshold is not None:
        cargs.extend(["--thr", str(threshold)])
    if num_samples is not None:
        cargs.extend(["--ns", str(num_samples)])
    if lambda_param is not None:
        cargs.extend(["--lambda", str(lambda_param)])
    if delta_param is not None:
        cargs.extend(["--delta", str(delta_param)])
    if alpha_param is not None:
        cargs.extend(["--alpha", str(alpha_param)])
    if seed_param is not None:
        cargs.extend(["--seed", str(seed_param)])
    if gfa_flag:
        cargs.append("--gfa")
    if savecoeff_flag:
    ...
    ret = QbootOutputs(
        ...
        output_files=execution.output_file(f"logdir/*"),
    )
    execution.run(cargs)
    return ret
