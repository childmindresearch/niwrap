...
POPP_METADATA = Metadata(
    id="237a9da8573aa7fc7906318f07b71b94c513f270",
    name="popp",
...
def popp(
    ...
    resp_column: float | int | None = None,
    cardiac_column: float | int | None = None,
    trigger_column: float | int | None = None,
    rvt_flag: bool = False,
    ...
    More information: https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/Popp
    ...
    if sampling_rate is not None:
        cargs.extend(["-s", str(sampling_rate)])
    if tr_value is not None:
        cargs.extend(["--tr", str(tr_value)])
    if resp_column is not None:
        cargs.extend(["--resp", str(resp_column)])
    if cardiac_column is not None:
        cargs.extend(["--cardiac", str(cardiac_column)])
    if trigger_column is not None:
        cargs.extend(["--trigger", str(trigger_column)])
    if rvt_flag:
        cargs.append("--rvt")
    if heart_rate_flag:
    ...
    if smooth_card is not None:
        cargs.extend(["--smoothcard", str(smooth_card)])
    if smooth_resp is not None:
        cargs.extend(["--smoothresp", str(smooth_resp)])
    if high_freq_cutoff is not None:
        cargs.extend(["--highfreqcutoff", str(high_freq_cutoff)])
    if low_freq_cutoff is not None:
        cargs.extend(["--lowfreqcutoff", str(low_freq_cutoff)])
    if init_thresh_c is not None:
        cargs.extend(["--initthreshc", str(init_thresh_c)])
    if n_thresh_c is not None:
        cargs.extend(["--nthreshc", str(n_thresh_c)])
    if init_thresh_r is not None:
        cargs.extend(["--initthreshr", str(init_thresh_r)])
    if n_thresh_r is not None:
        cargs.extend(["--nthreshr", str(n_thresh_r)])
    if invert_resp_flag:
    ...
    if load_card_phase is not None:
        cargs.extend(["--loadcardphase", execution.input_file(load_card_phase)])
    if load_resp_phase is not None:
        cargs.extend(["--loadrespphase", execution.input_file(load_resp_phase)])
    if vol_file is not None:
        cargs.extend(["--vol", execution.input_file(vol_file)])
    if start_sample is not None:
        cargs.extend(["--startingsample", str(start_sample)])
    if resp_add is not None:
        cargs.extend(["--respadd", resp_add])
    if resp_del is not None:
        cargs.extend(["--respdel", resp_del])
    if card_add is not None:
        cargs.extend(["--cardadd", card_add])
    if card_del is not None:
        cargs.extend(["--carddel", card_del])
    if verbose_flag:
        cargs.append("-v")
    if debug_flag:
        cargs.append("--debug")
    ...
    ret = PoppOutputs(
        ...
        processed_data_output=execution.output_file(f"{output_basename}_processed.txt"),
        timing_output=execution.output_file(f"{output_basename}_timing.txt", optional=True),
        rvt_data_output=execution.output_file(f"{output_basename}_rvt.txt", optional=True),
        heartrate_data_output=execution.output_file(f"{output_basename}_heartrate.txt", optional=True),
    )
    execution.run(cargs)
    return ret
