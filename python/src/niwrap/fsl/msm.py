...
def msm(
    ...
    levels: float | int | None = None,
    smoothout: float | int | None = None,
    help_: bool = False,
    verbose: bool = False,
    ...
    if refmesh is not None:
        cargs.extend(["--refmesh", execution.input_file(refmesh)])
    if indata is not None:
        cargs.extend(["--indata", execution.input_file(indata)])
    if refdata is not None:
        cargs.extend(["--refdata", execution.input_file(refdata)])
    if trans is not None:
        cargs.extend(["--trans", execution.input_file(trans)])
    if in_register is not None:
        cargs.extend(["--in_register", execution.input_file(in_register)])
    if inweight is not None:
        cargs.extend(["--inweight", execution.input_file(inweight)])
    if refweight is not None:
        cargs.extend(["--refweight", execution.input_file(refweight)])
    if format_ is not None:
        cargs.extend(["-f", format_])
    if conf is not None:
        cargs.extend(["--conf", execution.input_file(conf)])
    if levels is not None:
        cargs.extend(["--levels", str(levels)])
    if smoothout is not None:
        cargs.extend(["--smoothout", str(smoothout)])
    if help_:
        cargs.append("-h")
    if verbose:
        cargs.append("-v")
    ...
    ret = MsmOutputs(
        ...
        output_file=execution.output_file(f"{out}_output.ext", optional=True),
    )
    execution.run(cargs)
    return ret
