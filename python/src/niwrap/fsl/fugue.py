...
def fugue(
    ...
    ret = FugueOutputs(
        ...
        fmap_out_file_outfile=execution.output_file(f"{pathlib.Path(fmap_out_file).name}", optional=True) if fmap_out_file is not None else None,
        shift_out_file_outfile=execution.output_file(f"{pathlib.Path(shift_out_file).name}", optional=True) if shift_out_file is not None else None,
        unwarped_file_outfile=execution.output_file(f"{pathlib.Path(unwarped_file).name}", optional=True) if unwarped_file is not None else None,
        warped_file_outfile=execution.output_file(f"{pathlib.Path(warped_file).name}", optional=True) if warped_file is not None else None,
    )
    execution.run(cargs)
    return ret
