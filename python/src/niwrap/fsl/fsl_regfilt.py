...
FSL_REGFILT_METADATA = Metadata(
    id="5fc913e801e7759aa542fb28cbed443b7c26571f",
    name="fsl_regfilt",
...
def fsl_regfilt(
    ...
    freq_ic_smooth: float | int | None = None,
    fthresh: float | int | None = 0.15,
    fthresh2: float | int | None = 0.02,
    vn_flag: bool = False,
    ...
    if filter_ is not None:
        cargs.extend(["-f", filter_])
    if freq_filter_flag:
        cargs.append("-F")
    if freq_ic_flag:
        cargs.append("--freq_ic")
    if freq_ic_smooth is not None:
        cargs.extend(["--freq_ic_smooth", str(freq_ic_smooth)])
    if fthresh is not None:
        cargs.extend(["--fthresh", str(fthresh)])
    if fthresh2 is not None:
        cargs.extend(["--fthresh2", str(fthresh2)])
    if vn_flag:
        cargs.append("--vn")
    if verbose_flag:
        cargs.append("-v")
    ...
    if out_data is not None:
        cargs.extend(["--out_data", out_data])
    if out_mix is not None:
        cargs.extend(["--out_mix", out_mix])
    if out_vnscales is not None:
        cargs.extend(["--out_vnscales", out_vnscales])
    ret = FslRegfiltOutputs(
        root=execution.output_file("."),
        filtered_data=execution.output_file(f"{outfile}.nii.gz"),
        preprocessed_data=execution.output_file(f"{out_data}.nii.gz", optional=True) if out_data is not None else None,
        mixing_matrix=execution.output_file(f"{out_mix}.nii.gz", optional=True) if out_mix is not None else None,
        vnscales=execution.output_file(f"{out_vnscales}.nii.gz", optional=True) if out_vnscales is not None else None,
    )
    execution.run(cargs)
    return ret
