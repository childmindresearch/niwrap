...
FSL_MRSI_METADATA = Metadata(
    id="34193d6e96896802d533ed74733ece4a95b92639",
    name="fsl_mrsi",
    container_image_type="docker",
    container_image_index="index.docker.io",
    container_image_tag="fmrib/docker-fsl:latest",
)
...
def fsl_mrsi(
    ...
    output: str,
    algo: str | None = None,
    ignore_metab: list[str] = None,
    keep_metab: list[str] = None,
    combine_metab: list[str] = None,
    ppmlim: list[float | int] = None,
    h2o: InputPathType | None = None,
    baseline_order: float | int | None = None,
    metab_groups: list[str] = None,
    add_mm: bool = False,
    lorentzian: bool = False,
    ind_scale: list[str] = None,
    disable_mh_priors: bool = False,
    te: float | int | None = None,
    tr: float | int | None = None,
    tissue_frac: list[InputPathType] = None,
    internal_ref: list[str] = None,
    wref_metabolite: str | None = None,
    ref_protons: float | int | None = None,
    ref_int_limits: list[float | int] = None,
    h2o_scale: float | int | None = None,
    report: bool = False,
    verbose: bool = False,
    ...
    if ppmlim is not None and not (len(ppmlim) <= 2): 
        raise ValueError(f"Length of 'ppmlim' must be less than 2 but was {len(ppmlim)}")
    ...
    if ref_int_limits is not None and not (len(ref_int_limits) <= 2): 
        raise ValueError(f"Length of 'ref_int_limits' must be less than 2 but was {len(ref_int_limits)}")
    ...
    cargs.append(output)
    if algo is not None:
        cargs.extend(["--algo", algo])
    if ignore_metab is not None:
        cargs.extend(["--ignore", *ignore_metab])
    if keep_metab is not None:
        cargs.extend(["--keep", *keep_metab])
    if combine_metab is not None:
        cargs.extend(["--combine", *combine_metab])
    if ppmlim is not None:
        cargs.extend(["--ppmlim", *map(str, ppmlim)])
    if h2o is not None:
        cargs.extend(["--h2o", execution.input_file(h2o)])
    if baseline_order is not None:
        cargs.extend(["--baseline_order", str(baseline_order)])
    if metab_groups is not None:
        cargs.extend(["--metab_groups", *metab_groups])
    if add_mm:
        cargs.append("--add_MM")
    if lorentzian:
        cargs.append("--lorentzian")
    if ind_scale is not None:
        cargs.extend(["--ind_scale", *ind_scale])
    ...
    if tissue_frac is not None:
        cargs.extend(["--tissue_frac", *[execution.input_file(f) for f in tissue_frac]])
    if internal_ref is not None:
        cargs.extend(["--internal_ref", *internal_ref])
    if wref_metabolite is not None:
        cargs.extend(["--wref_metabolite", wref_metabolite])
    if ref_protons is not None:
        cargs.extend(["--ref_protons", str(ref_protons)])
    if ref_int_limits is not None:
        cargs.extend(["--ref_int_limits", *map(str, ref_int_limits)])
    if h2o_scale is not None:
        cargs.extend(["--h2o_scale", str(h2o_scale)])
    if report:
        cargs.append("--report")
    if verbose:
