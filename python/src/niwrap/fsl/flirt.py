...
def flirt(
    ...
    ret = FlirtOutputs(
        ...
        out_file=execution.output_file(f"{_rstrip(pathlib.Path(in_file).name, ['.nii.gz', '.nii'])}_flirt.nii", optional=True),
        out_matrix_file=execution.output_file(f"{_rstrip(pathlib.Path(in_file).name, ['.nii.gz', '.nii'])}_flirt.mat", optional=True),
    )
    execution.run(cargs)
    return ret
