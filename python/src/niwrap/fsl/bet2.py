...
class BetBrainExtractionToolOutputs(typing.NamedTuple):
    """
    Output object returned when calling `bet__brain_extraction_tool_(...)`.
    """
    root: OutputPathType
...
def bet__brain_extraction_tool_(
    input_fileroot: str,
    output_fileroot: str,
    ...
    smooth_factor: float | int | None = None,
    ...
    execution = runner.start_execution(BET__BRAIN_EXTRACTION_TOOL__METADATA)
    cargs = []
    cargs.append("bet2")
    ...
    if vertical_gradient is not None:
        cargs.extend(["-g", str(vertical_gradient)])
    ...
    if smooth_factor is not None:
        cargs.extend(["-w", str(smooth_factor)])
    if threshold_flag:
        cargs.append("-t")
    if verbose_flag:
        cargs.append("-v")
    if help_flag:
        cargs.append("-h")
    ret = BetBrainExtractionToolOutputs(
        root=execution.output_file("."),
        output_mask=execution.output_file(f"{output_fileroot}_mask.nii.gz", optional=True),
        output_skull=execution.output_file(f"{output_fileroot}_skull.nii.gz", optional=True),
        output_mesh=execution.output_file(f"{output_fileroot}_mesh.vtk", optional=True),
        output_overlay=execution.output_file(f"{output_fileroot}_overlay.nii.gz", optional=True),
    )
    execution.run(cargs)
    return ret
...
__all__ = [
    "BET__BRAIN_EXTRACTION_TOOL__METADATA",
    "BetBrainExtractionToolOutputs",
    "bet__brain_extraction_tool_",
]
