...
def fsl_glm(
    ...
    if output_file is not None:
        cargs.extend(["-o", execution.input_file(output_file)])
    ...
    if output_copes is not None:
        cargs.extend(["--out_cope", execution.input_file(output_copes)])
    if output_zstats is not None:
        cargs.extend(["--out_z", execution.input_file(output_zstats)])
    if output_tstats is not None:
        cargs.extend(["--out_t", execution.input_file(output_tstats)])
    if output_pvals is not None:
        cargs.extend(["--out_p", execution.input_file(output_pvals)])
    if output_fvals is not None:
        cargs.extend(["--out_f", execution.input_file(output_fvals)])
    if output_pfvals is not None:
        cargs.extend(["--out_pf", execution.input_file(output_pfvals)])
    if output_residuals is not None:
        cargs.extend(["--out_res", execution.input_file(output_residuals)])
    if output_varcb is not None:
        cargs.extend(["--out_varcb", execution.input_file(output_varcb)])
    if output_sigsq is not None:
        cargs.extend(["--out_sigsq", execution.input_file(output_sigsq)])
    if output_data is not None:
        cargs.extend(["--out_data", execution.input_file(output_data)])
    if output_vnscales is not None:
        cargs.extend(["--out_vnscales", execution.input_file(output_vnscales)])
    if vx_text is not None:
        cargs.extend(["--vxt", *vx_text])
    if vx_images is not None:
        cargs.extend(["--vxf", *[execution.input_file(f) for f in vx_images]])
    if help_flag:
        cargs.append("-h")
    ret = FslGlmOutputs(
        root=execution.output_file("."),
        output_file_out=execution.output_file(f"{pathlib.Path(output_file).name}.nii.gz", optional=True) if output_file is not None else None,
        output_copes_out=execution.output_file(f"{pathlib.Path(output_copes).name}.nii.gz", optional=True) if output_copes is not None else None,
        output_zstats_out=execution.output_file(f"{pathlib.Path(output_zstats).name}.nii.gz", optional=True) if output_zstats is not None else None,
        output_tstats_out=execution.output_file(f"{pathlib.Path(output_tstats).name}.nii.gz", optional=True) if output_tstats is not None else None,
        output_pvals_out=execution.output_file(f"{pathlib.Path(output_pvals).name}.nii.gz", optional=True) if output_pvals is not None else None,
        output_fvals_out=execution.output_file(f"{pathlib.Path(output_fvals).name}.nii.gz", optional=True) if output_fvals is not None else None,
        output_pfvals_out=execution.output_file(f"{pathlib.Path(output_pfvals).name}.nii.gz", optional=True) if output_pfvals is not None else None,
        output_residuals_out=execution.output_file(f"{pathlib.Path(output_residuals).name}.nii.gz", optional=True) if output_residuals is not None else None,
        output_varcb_out=execution.output_file(f"{pathlib.Path(output_varcb).name}.nii.gz", optional=True) if output_varcb is not None else None,
        output_sigsq_out=execution.output_file(f"{pathlib.Path(output_sigsq).name}.nii.gz", optional=True) if output_sigsq is not None else None,
        output_data_out=execution.output_file(f"{pathlib.Path(output_data).name}.nii.gz", optional=True) if output_data is not None else None,
        output_vnscales_out=execution.output_file(f"{pathlib.Path(output_vnscales).name}.nii.gz", optional=True) if output_vnscales is not None else None,
    )
    execution.run(cargs)
    return ret
