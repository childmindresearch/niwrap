...
def siena_flirt(
    ...
    ret = SienaFlirtOutputs(
        ...
        output_transform_matrix=execution.output_file(f"{input1_fileroot}_to_{input2_fileroot}_flirt.mat"),
        output_registered_image=execution.output_file(f"{input1_fileroot}_to_{input2_fileroot}_flirt.nii.gz"),
    )
    execution.run(cargs)
    return ret
