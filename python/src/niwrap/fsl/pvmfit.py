...
def pvmfit(
    ...
    number_of_fibres: float | int | None = 1,
    ...
    if number_of_fibres is not None:
        cargs.extend(["-n", str(number_of_fibres)])
    ...
    ret = PvmfitOutputs(
        ...
        output_file=execution.output_file(f"{output_basename}.nii.gz", optional=True) if output_basename is not None else None,
        bic_file=execution.output_file(f"{output_basename}_BIC.nii.gz", optional=True) if output_basename is not None else None,
    )
    execution.run(cargs)
    return ret
