...
def film_cifti(
    ...
    susan_threshold: float | int | None = None,
    susan_extent: float | int | None = None,
    ...
    if susan_threshold is not None:
        cargs.extend(["--st", str(susan_threshold)])
    if susan_extent is not None:
        cargs.extend(["--sm", str(susan_extent)])
    if surface_sigma is not None:
        cargs.extend(["--ss", str(surface_sigma)])
    if surface_extent is not None:
        cargs.extend(["--se", str(surface_extent)])
    if film_options is not None:
        cargs.extend(["--filmOptions", film_options])
    ret = FilmCiftiOutputs(
        root=execution.output_file("."),
        output_results=execution.output_file(f"{basename}_results.nii.gz"),
    )
    execution.run(cargs)
    return ret
