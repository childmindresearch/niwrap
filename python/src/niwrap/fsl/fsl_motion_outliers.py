...
def fsl_motion_outliers(
    ...
    abs_thresh: float | int | None = None,
    ...
    if save_metric_file is not None:
        cargs.extend(["-s", save_metric_file])
    if save_metric_plot is not None:
        cargs.extend(["-p", save_metric_plot])
    if temp_path is not None:
        cargs.extend(["-t", temp_path])
    if refrms_flag:
        cargs.append("--refrms")
    if dvars_flag:
        cargs.append("--dvars")
    ...
    if abs_thresh is not None:
        cargs.extend(["--thresh", str(abs_thresh)])
    if no_moco_flag:
        cargs.append("--nomoco")
    if dummy_scans is not None:
        cargs.extend(["--dummy", str(dummy_scans)])
    if verbose_flag:
        cargs.append("-v")
    ret = FslMotionOutliersOutputs(
        root=execution.output_file("."),
        output_confound_file=execution.output_file(f"{output_confound_file}"),
        metric_text_file=execution.output_file(f"{save_metric_file}", optional=True) if save_metric_file is not None else None,
        metric_plot_file=execution.output_file(f"{save_metric_plot}", optional=True) if save_metric_plot is not None else None,
    )
    execution.run(cargs)
    return ret
