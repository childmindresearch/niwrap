...
FABBER_T1_METADATA = Metadata(
    id="55375ed72258eeb6c37147e89ef7264e0b8efa03",
    name="fabber_t1",
    container_image_type="docker",
    container_image_index="index.docker.io",
    container_image_tag="fmrib/fabber:latest",
)
...
def fabber_t1(
    ...
    cargs.extend(["--method", method])
    cargs.extend(["--model", model])
    if data_mult is not None:
        cargs.extend(["--data<n>", *[execution.input_file(f) for f in data_mult]])
    if data_order is not None:
        cargs.extend(["--data-order", data_order])
    if mask is not None:
        cargs.extend(["--mask", execution.input_file(mask)])
    cargs.append("[MASKED_TIME_POINTS]")
    if supp_data is not None:
        cargs.extend(["--suppdata", execution.input_file(supp_data)])
    if overwrite:
    ...
    if load_models is not None:
        cargs.extend(["--loadmodels", execution.input_file(load_models)])
    if evaluate is not None:
        cargs.extend(["--evaluate", evaluate])
    if evaluate_params is not None:
        cargs.extend(["--evaluate-params", evaluate_params])
    if evaluate_nt is not None:
        cargs.extend(["--evaluate-nt", str(evaluate_nt)])
    if dump_param_names:
    ...
    if optfile is not None:
        cargs.extend(["--optfile", execution.input_file(optfile)])
    if debug:
        cargs.append("--debug")
    ret = FabberT1Outputs(
        root=execution.output_file("."),
        param_names=execution.output_file(f"{output}/paramnames.txt", optional=True),
        model_fit=execution.output_file(f"{output}/model_prediction.nii.gz", optional=True),
        residuals=execution.output_file(f"{output}/residuals.nii.gz", optional=True),
        model_extras=execution.output_file(f"{output}/model_extras.nii.gz", optional=True),
        mvn_distributions=execution.output_file(f"{output}/mvn_distributions.nii.gz", optional=True),
        param_means=execution.output_file(f"{output}/param_means.nii.gz", optional=True),
        param_stds=execution.output_file(f"{output}/param_stds.nii.gz", optional=True),
        param_vars=execution.output_file(f"{output}/param_vars.nii.gz", optional=True),
        param_zstats=execution.output_file(f"{output}/param_zstats.nii.gz", optional=True),
        noise_means=execution.output_file(f"{output}/noise_means.nii.gz", optional=True),
        noise_stds=execution.output_file(f"{output}/noise_stds.nii.gz", optional=True),
        free_energy=execution.output_file(f"{output}/free_energy.nii.gz", optional=True),
    )
    execution.run(cargs)
    return ret
