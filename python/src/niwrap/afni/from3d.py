...
def from3d(
    ...
    raw: bool = False,
    float_: bool = False,
    zfirst: float | int | None = None,
    zlast: float | int | None = None,
    tfirst: float | int | None = None,
    tlast: float | int | None = None,
    ...
    if zfirst is not None:
        cargs.extend(["-zfirst", str(zfirst)])
    if zlast is not None:
        cargs.extend(["-zlast", str(zlast)])
    if tfirst is not None:
        cargs.extend(["-tfirst", str(tfirst)])
    if tlast is not None:
        cargs.extend(["-tlast", str(tlast)])
    cargs.append("-input")
    ...
    ret = From3dOutputs(
        ...
        extracted_images=execution.output_file(f"{prefix}*.img"),
    )
    execution.run(cargs)
    return ret
