...
def v_3d_local_bistat(
    ...
    histpow: float | int | None = None,
    histbin: float | int | None = None,
    ...
    cargs.extend(["-nbhd", nbhd])
    cargs.extend(["-stat", *stats])
    if mask is not None:
        cargs.extend(["-mask", execution.input_file(mask)])
    if automask:
        cargs.append("-automask")
    if weight is not None:
        cargs.extend(["-weight", execution.input_file(weight)])
    cargs.extend(["-prefix", prefix])
    if histpow is not None:
        cargs.extend(["-histpow", str(histpow)])
    if histbin is not None:
        cargs.extend(["-histbin", str(histbin)])
    if hclip1 is not None:
        cargs.extend(["-hclip1", *hclip1])
    if hclip2 is not None:
        cargs.extend(["-hclip2", *hclip2])
