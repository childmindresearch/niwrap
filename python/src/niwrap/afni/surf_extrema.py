...
def surf_extrema(
    ...
    if hood is not None:
        cargs.extend(["-nbhd_rad", str(hood)])
    if thresh is not None:
        cargs.extend(["-thresh", str(thresh)])
    if gthresh is not None:
        cargs.extend(["-gthresh", str(gthresh)])
    if gscale is not None:
        cargs.extend(["-gscale", gscale])
    if extype is not None:
        cargs.extend(["-extype", extype])
    cargs.extend(["-prefix", prefix])
    if table is not None:
        cargs.extend(["-table", table])
    ret = SurfExtremaOutputs(
        root=execution.output_file("."),
        output_grd=execution.output_file(f"{prefix}.grd"),
        output_ext=execution.output_file(f"{prefix}.ext"),
    )
    execution.run(cargs)
    return ret
