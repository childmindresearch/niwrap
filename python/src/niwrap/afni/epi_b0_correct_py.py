...
def epi_b0_correct(
    ...
    epi_pe_bwpp: float | int | None = None,
    epi_pe_echo_sp: float | int | None = None,
    epi_pe_vox_dim: float | int | None = None,
    scale_freq: float | int | None = None,
    ...
    mask_dilate: list[float | int] | None = None,
    no_clean: bool = False,
    ...
    cargs.append(prefix)
    cargs.extend(["-in_freq", execution.input_file(input_freq)])
    cargs.extend(["-in_epi", execution.input_file(input_epi)])
    if input_mask is not None:
        cargs.extend(["-in_mask", execution.input_file(input_mask)])
    if input_magn is not None:
        cargs.extend(["-in_magn", execution.input_file(input_magn)])
    if input_anat is not None:
        cargs.extend(["-in_anat", execution.input_file(input_anat)])
    if input_json is not None:
        cargs.extend(["-in_epi_json", execution.input_file(input_json)])
    cargs.extend(["-epi_pe_dir", epi_pe_dir])
    if epi_pe_bwpp is not None:
        cargs.extend(["-epi_pe_bwpp", str(epi_pe_bwpp)])
    if epi_pe_echo_sp is not None:
        cargs.extend(["-epi_pe_echo_sp", str(epi_pe_echo_sp)])
    if epi_pe_vox_dim is not None:
        cargs.extend(["-epi_pe_voxdim", str(epi_pe_vox_dim)])
    if scale_freq is not None:
        cargs.extend(["-scale_freq", str(scale_freq)])
    if out_cmds is not None:
        cargs.extend(["-out_cmds", out_cmds])
    if out_pars is not None:
        cargs.extend(["-out_pars", out_pars])
    if wdir_name is not None:
        cargs.extend(["-wdir_name", wdir_name])
    if blur_sigma is not None:
        cargs.extend(["-blur_sigma", str(blur_sigma)])
    if do_recenter_freq is not None:
        cargs.extend(["-do_recenter_freq", do_recenter_freq])
    if mask_dilate is not None:
        cargs.extend(["-mask_dilate", *map(str, mask_dilate)])
    if no_clean:
    ...
    ret = EpiB0CorrectOutputs(
        ...
        warp_dset=execution.output_file(f"{prefix}_WARP.nii.gz", optional=True),
        cmds_script=execution.output_file(f"{prefix}_cmds.tcsh", optional=True),
        params_txt=execution.output_file(f"{prefix}_pars.txt", optional=True),
        unwarped_epi=execution.output_file(f"{prefix}_unwarped.nii.gz", optional=True),
        qc_image_dir=execution.output_file(f"{prefix}_QC/", optional=True),
    )
    execution.run(cargs)
    return ret
