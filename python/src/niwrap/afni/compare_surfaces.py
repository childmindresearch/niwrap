...
def compare_surfaces(
    ...
    one_node: float | int | None = None,
    node_range: list[float | int] | None = None,
    ...
    if node_range is not None and not (len(node_range) <= 2): 
        raise ValueError(f"Length of 'node_range' must be less than 2 but was {len(node_range)}")
    ...
    cargs.extend(["-hemi", hemisphere])
    cargs.extend(["-sv1", execution.input_file(volume_parent_1)])
    cargs.extend(["-sv2", execution.input_file(volume_parent_2)])
    if file_prefix is not None:
        cargs.extend(["-prefix", file_prefix])
    if one_node is not None:
        cargs.extend(["-onenode", str(one_node)])
    if node_range is not None:
        cargs.extend(["-noderange", *map(str, node_range)])
    ...
    if set_environment_variable is not None:
        cargs.extend(["-setenv", set_environment_variable])
    if trace_:
        cargs.append("-trace")
    if extreme_trace:
        cargs.append("-TRACE")
    ...
    ret = CompareSurfacesOutputs(
        ...
        distance_output_file=execution.output_file(f"{file_prefix}_distance.txt", optional=True) if file_prefix is not None else None,
        color_output_file=execution.output_file(f"{file_prefix}_color.txt", optional=True) if file_prefix is not None else None,
    )
    execution.run(cargs)
    return ret
