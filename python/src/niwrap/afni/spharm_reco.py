...
class SphericalHarmonicsReconstructionOutputs(typing.NamedTuple):
    """
    Output object returned when calling `spherical_harmonics_reconstruction(...)`.
    """
    root: OutputPathType
    """Output root folder. This is the root folder for all outputs."""


def spherical_harmonics_reconstruction(
    input_surface: str,
    decomposition_order: float | int,
    bases_prefix: str,
    ...
    Returns:
        NamedTuple of outputs (described in `SphericalHarmonicsReconstructionOutputs`).
    """
    ...
    execution = runner.start_execution(SPHERICAL_HARMONICS_RECONSTRUCTION_METADATA)
    cargs = []
    cargs.append("SpharmReco")
    cargs.extend(["-i_TYPE", input_surface])
    cargs.extend(["-l", str(decomposition_order)])
    cargs.extend(["-bases_prefix", bases_prefix])
    cargs.extend(["-coef", *[execution.input_file(f) for f in coefficients]])
    if output_prefix is not None:
        cargs.extend(["-prefix", output_prefix])
    if output_surface is not None:
        cargs.extend(["-o_TYPE", *output_surface])
    if debug is not None:
        cargs.extend(["-debug", str(debug)])
    if smoothing is not None:
        cargs.extend(["-sigma", str(smoothing)])
    ret = SphericalHarmonicsReconstructionOutputs(
...
__all__ = [
    "SPHERICAL_HARMONICS_RECONSTRUCTION_METADATA",
    "SphericalHarmonicsReconstructionOutputs",
    "spherical_harmonics_reconstruction",
]
