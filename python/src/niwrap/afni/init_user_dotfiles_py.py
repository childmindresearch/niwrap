...
def init_user_dotfiles_py(
    ...
    if dot_files_list is not None:
        cargs.extend(["-dflist", *dot_files_list])
    if dir_bin is not None:
        cargs.extend(["-dir_bin", dir_bin])
    if dir_dot is not None:
        cargs.extend(["-dir_dot", dir_dot])
    if do_updates is not None:
        cargs.extend(["-do_updates", *do_updates])
    if dry_run:
    ...
    if shell_list is not None:
        cargs.extend(["-shell_list", *shell_list])
    if test:
        cargs.append("-test")
