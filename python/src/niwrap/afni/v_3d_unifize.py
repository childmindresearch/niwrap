...
def v_3d_unifize(
    ...
    * This program replaces the older (and very different) 3dUniformize, which
    ...
    ret = V3dUnifizeOutputs(
        ...
        out_file=execution.output_file(f"{pathlib.Path(in_file).name}_unifized", optional=True),
        out_file_=execution.output_file(f"out_file", optional=True),
        scale_file_outfile=execution.output_file(f"{pathlib.Path(scale_file).name}", optional=True) if scale_file is not None else None,
    )
    execution.run(cargs)
    return ret
