...
def surf_smooth(
    ...
    target_fwhm: float | int | None = None,
    fwhm: float | int | None = None,
    number_iterations: float | int | None = None,
    output_file: InputPathType | None = None,
    band_pass_frequency: float | int | None = None,
    ...
    dbg_node: float | int | None = None,
    use_neighbors_outside_mask: bool = False,
    talk_suma: bool = False,
    refresh_rate: float | int | None = None,
    ...
    ret = SurfSmoothOutputs(
        ...
        out_file=execution.output_file(f"{pathlib.Path(output_file).name}") if output_file is not None else None,
    )
    execution.run(cargs)
    return ret
