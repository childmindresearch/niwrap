...
def uber_ttest(
    ...
    beta_a: float | int | None = None,
    beta_b: float | int | None = None,
    tstat_a: float | int | None = None,
    tstat_b: float | int | None = None,
    ...
    if program is not None:
        cargs.extend(["-program", program])
    if print_script:
        cargs.append("-print_script")
    if save_script is not None:
        cargs.extend(["-save_script", save_script])
    if mask is not None:
        cargs.extend(["-mask", execution.input_file(mask)])
    if set_name_a is not None:
        cargs.extend(["-set_name_A", set_name_a])
    if set_name_b is not None:
        cargs.extend(["-set_name_B", set_name_b])
    if dsets_a is not None:
        cargs.extend(["-dsets_A", dsets_a])
    if dsets_b is not None:
        cargs.extend(["-dsets_B", dsets_b])
    if beta_a is not None:
        cargs.extend(["-beta_A", str(beta_a)])
    if beta_b is not None:
        cargs.extend(["-beta_B", str(beta_b)])
    if tstat_a is not None:
        cargs.extend(["-tstat_A", str(tstat_a)])
    if tstat_b is not None:
        cargs.extend(["-tstat_B", str(tstat_b)])
    if results_dir is not None:
        cargs.extend(["-results_dir", results_dir])
    ret = UberTtestOutputs(
        root=execution.output_file("."),
        output_script=execution.output_file(f"{save_script}", optional=True) if save_script is not None else None,
    )
    execution.run(cargs)
    return ret
