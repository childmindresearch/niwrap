...
def fat_proc_map_to_dti(
    ...
    cargs.extend(["-source", execution.input_file(source)])
    cargs.extend(["-base", execution.input_file(base)])
    cargs.extend(["-prefix", prefix])
    if followers_nn is not None:
        cargs.extend(["-followers_NN", *[execution.input_file(f) for f in followers_nn]])
    if followers_wsinc5 is not None:
        cargs.extend(["-followers_wsinc5", *[execution.input_file(f) for f in followers_wsinc5]])
    if followers_surf is not None:
        cargs.extend(["-followers_surf", *[execution.input_file(f) for f in followers_surf]])
    if followers_ndset is not None:
        cargs.extend(["-followers_ndset", *[execution.input_file(f) for f in followers_ndset]])
    if followers_spec is not None:
        cargs.extend(["-followers_spec", *[execution.input_file(f) for f in followers_spec]])
