# This file was auto generated by Styx.
# Do not edit this file directly.

from styxdefs import *
import pathlib
import typing

_DEBLANK_FILE_NAMES_METADATA = Metadata(
    id="037e8504a76d03844cb09dc0dad656f071af0f74",
    name="@DeblankFileNames",
)


class DeblankFileNamesOutputs(typing.NamedTuple):
    """
    Output object returned when calling `_deblank_file_names(...)`.
    """
    root: OutputPathType
    """Output root folder. This is the root folder for all outputs."""


def _deblank_file_names(
    move: bool = False,
    nobrac: bool = False,
    demo_set: bool = False,
    echo: bool = False,
    help_: bool = False,
    files: list[InputPathType] | None = None,
    runner: Runner | None = None,
) -> DeblankFileNamesOutputs:
    """
    @DeblankFileNames by Unknown.
    
    A script to remove blanks and other annoying characters ([], ()) from
    filenames.
    
    Args:
        move: Actually rename the files (opposite of -dry_run).
        nobrac: Do not replace () and [] in filenames, just spaces.
        demo_set: Create a toy directory with bad names for testing.
        echo: Turn on script echo.
        help_: Display help message.
        files: Specify files to fix as opposed to letting it fix all the names\
            in the current directory.
        runner: Command runner.
    Returns:
        NamedTuple of outputs (described in `DeblankFileNamesOutputs`).
    """
    runner = runner or get_global_runner()
    execution = runner.start_execution(_DEBLANK_FILE_NAMES_METADATA)
    cargs = []
    cargs.append("@DeblankFileNames")
    if move:
        cargs.append("-move")
    if nobrac:
        cargs.append("-nobrac")
    if demo_set:
        cargs.append("-demo_set")
    if echo:
        cargs.append("-echo")
    if help_:
        cargs.append("-help")
    cargs.append("[FILES...]")
    ret = DeblankFileNamesOutputs(
        root=execution.output_file("."),
    )
    execution.run(cargs)
    return ret


__all__ = [
    "DeblankFileNamesOutputs",
    "_DEBLANK_FILE_NAMES_METADATA",
    "_deblank_file_names",
]
