...
def fat_mvm_scripter_py(
    ...
    cargs.extend(["--table", execution.input_file(table)])
    cargs.extend(["--log", execution.input_file(log)])
    if vars_ is not None:
        cargs.extend(["--vars", vars_])
    if file_vars is not None:
        cargs.extend(["--file_vars", execution.input_file(file_vars)])
    if pars is not None:
        cargs.extend(["--Pars", pars])
    if file_pars is not None:
        cargs.extend(["--File_Pars", execution.input_file(file_pars)])
    if rois is not None:
        cargs.extend(["--rois", rois])
    if file_rois is not None:
        cargs.extend(["--file_rois", execution.input_file(file_rois)])
    if no_posthoc:
        cargs.append("--no_posthoc")
    if na_warn_off:
        cargs.append("--NA_warn_off")
    if subnet_pref is not None:
        cargs.extend(["--subnet_pref", subnet_pref])
    if cat_pair_off:
        cargs.append("--cat_pair_off")
    ret = FatMvmScripterPyOutputs(
        root=execution.output_file("."),
        generated_script=execution.output_file(f"{prefix}_scri.tcsh", optional=True),
        results_file=execution.output_file(f"{prefix}_MVM.txt", optional=True),
    )
    execution.run(cargs)
    return ret
