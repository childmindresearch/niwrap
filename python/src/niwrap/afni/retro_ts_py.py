...
def retro_ts_py(
    ...
    if resp_file is not None:
        cargs.extend(["-r", execution.input_file(resp_file)])
    if card_file is not None:
        cargs.extend(["-c", execution.input_file(card_file)])
    if phys_fs is not None:
        cargs.extend(["-p", str(phys_fs)])
    cargs.extend(["-n", str(num_slices)])
    cargs.extend(["-v", str(volume_tr)])
    if phys_file is not None:
        cargs.extend(["-phys_file", execution.input_file(phys_file)])
    if phys_json is not None:
        cargs.extend(["-phys_json", execution.input_file(phys_json)])
    if prefix is not None:
        cargs.extend(["-prefix", prefix])
    if rvt_shifts is not None:
        cargs.extend(["-rvt_shifts", rvt_shifts])
    if rvt_out:
        cargs.append("-rvt_out")
    if resp_cutoff_freq is not None:
        cargs.extend(["-respiration_cutoff_frequency", str(resp_cutoff_freq)])
    if cardiac_cutoff_freq is not None:
        cargs.extend(["-cardiac_cutoff_frequency", str(cardiac_cutoff_freq)])
    if cardiac_out:
        cargs.append("-cardiac_out")
    if respiration_out:
        cargs.append("-respiration_out")
    if interp_style is not None:
        cargs.extend(["-interpolation_style", interp_style])
    if fir_order is not None:
        cargs.extend(["-fir_order", str(fir_order)])
    if quiet:
        cargs.append("-quiet")
    if demo:
        cargs.append("-demo")
    ...
    if slice_offset is not None:
        cargs.extend(["-slice_offset", slice_offset])
    if slice_major is not None:
        cargs.extend(["-slice_major", str(slice_major)])
    if slice_order is not None:
        cargs.extend(["-slice_order", slice_order])
    if zero_phase_offset:
        cargs.append("-zero_phase_offset")
    if legacy_transform is not None:
        cargs.extend(["-legacy_transform", str(legacy_transform)])
    ret = RetroTsPyOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{prefix}.slibase.1D") if prefix is not None else None,
    )
    execution.run(cargs)
    return ret
