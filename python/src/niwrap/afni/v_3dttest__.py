...
def v_3dttest__(
    ...
    permute: bool = False,
    etac: bool = False,
    etac_blur: list[float | int] | None = None,
    ...
    ret = V3dttestOutputs(
        ...
        out_file=execution.output_file(f"{prefix}.nii.gz", optional=True) if prefix is not None else None,
        residuals=execution.output_file(f"{resid}.nii.gz", optional=True) if resid is not None else None,
    )
    execution.run(cargs)
    return ret
