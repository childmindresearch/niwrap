...
def v_3d_zregrid(
    ...
    z_thickness: float | int | None = None,
    slice_count: float | int | None = None,
    z_size: float | int | None = None,
    ...
    if z_thickness is not None:
        cargs.extend(["-dz", str(z_thickness)])
    if slice_count is not None:
        cargs.extend(["-nz", str(slice_count)])
    if z_size is not None:
        cargs.extend(["-zsize", str(z_size)])
    ...
    ret = V3dZregridOutputs(
        ...
        outfile_head=execution.output_file(f"{prefix}+orig.HEAD") if prefix is not None else None,
        outfile_brik=execution.output_file(f"{prefix}+orig.BRIK") if prefix is not None else None,
    )
    execution.run(cargs)
    return ret
