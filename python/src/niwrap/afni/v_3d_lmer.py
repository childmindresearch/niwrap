...
def v_3d_lmer(
    ...
    model: str,
    prefix: str,
    bound_lower: float | int | None = None,
    bound_upper: float | int | None = None,
    cio: bool = False,
    ...
    if glf_code is not None:
        cargs.extend(["-glfCode", glf_code])
    if glt_code is not None:
        cargs.extend(["-gltCode", glt_code])
    if help_:
        cargs.append("-help")
    if input_file_column is not None:
        cargs.extend(["-IF", input_file_column])
    if jobs is not None:
        cargs.extend(["-jobs", str(jobs)])
    if mask is not None:
    ...
    if qvars is not None:
        cargs.extend(["-qVars", qvars])
    if resid is not None:
        cargs.extend(["-resid", resid])
    if rio:
        cargs.append("-Rio")
    if show_options:
        cargs.append("-show_allowed_options")
    if ss_type is not None:
        cargs.extend(["-SS_type", str(ss_type)])
    if trr:
        cargs.append("-TRR")
    if vvar_centers is not None:
        cargs.extend(["-vVarCenters", vvar_centers])
    if vvars is not None:
        cargs.extend(["-vVars", vvars])
    ret = V3dLmerOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{prefix}.nii.gz"),
        residuals_file=execution.output_file(f"{resid}.nii.gz", optional=True) if resid is not None else None,
    )
    execution.run(cargs)
    return ret
