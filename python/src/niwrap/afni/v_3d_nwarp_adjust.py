...
def v_3d_nwarp_adjust(
    ...
    cargs.extend(["-nwarp", *[execution.input_file(f) for f in input_warps]])
    if source_datasets is not None:
        cargs.extend(["-source", *[execution.input_file(f) for f in source_datasets]])
    if output_prefix is not None:
        cargs.extend(["-prefix", output_prefix])
    ret = V3dNwarpAdjustOutputs(
        root=execution.output_file("."),
        output_brik=execution.output_file(f"{output_prefix}+tlrc.BRIK", optional=True) if output_prefix is not None else None,
        output_head=execution.output_file(f"{output_prefix}+tlrc.HEAD", optional=True) if output_prefix is not None else None,
    )
    execution.run(cargs)
    return ret
