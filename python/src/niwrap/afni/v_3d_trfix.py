...
def v_3d_trfix(
    ...
    if tr_list is not None:
        cargs.extend(["-TRlist", execution.input_file(tr_list)])
    cargs.append("-TIMElist")
    if time_list is not None:
        cargs.extend(["-TIMElist", execution.input_file(time_list)])
    cargs.append("-prefix")
    cargs.extend(["-prefix", prefix])
    cargs.append("-TRout")
    if output_tr is not None:
        cargs.extend(["-TRout", str(output_tr)])
    ret = V3dTrfixOutputs(
        root=execution.output_file("."),
        output_file_head=execution.output_file(f"{prefix}+orig.HEAD"),
        output_file_brik=execution.output_file(f"{prefix}+orig.BRIK"),
    )
    execution.run(cargs)
    return ret
