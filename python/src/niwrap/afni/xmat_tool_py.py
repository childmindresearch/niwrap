...
def xmat_tool_py(
    ...
    chrono: bool = False,
    cormat_cutoff: float | int | None = None,
    cosmat_cutoff: float | int | None = None,
    ...
    if load_xmat is not None:
        cargs.extend(["-load_xmat", execution.input_file(load_xmat)])
    if load_1d is not None:
        cargs.extend(["-load_1D", execution.input_file(load_1d)])
    if choose_cols is not None:
        cargs.extend(["-choose_cols", choose_cols])
    if choose_nonzero_cols:
        cargs.append("-choose_nonzero_cols")
    if chrono:
        cargs.append("-chrono")
    if cormat_cutoff is not None:
        cargs.extend(["-cormat_cutoff", str(cormat_cutoff)])
    if cosmat_cutoff is not None:
        cargs.extend(["-cosmat_cutoff", str(cosmat_cutoff)])
    if cosmat_motion:
    ...
    ret = XmatToolPyOutputs(
        ...
        output_fitts=execution.output_file(f"fitts.1D", optional=True),
    )
    execution.run(cargs)
    return ret
