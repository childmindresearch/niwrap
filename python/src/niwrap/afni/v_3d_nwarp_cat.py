...
def v_3d_nwarp_cat(
    ...
    extra_padding: float | int | None = None,
    ...
    if interpolation is not None:
        cargs.extend(["-interp", interpolation])
    if verbosity:
        cargs.append("-verb")
    cargs.append("-prefix")
    cargs.extend(["-prefix", output_prefix])
    if space_marker is not None:
        cargs.extend(["-space", space_marker])
    cargs.append("-warp1")
    cargs.extend(["-warp1", execution.input_file(warp1)])
    cargs.append("-warp2")
    cargs.extend(["-warp2", execution.input_file(warp2)])
    ...
    if extra_padding is not None:
        cargs.extend(["-expad", str(extra_padding)])
    ret = V3dNwarpCatOutputs(
        root=execution.output_file("."),
        output_matrix=execution.output_file(f"{output_prefix}.aff12.1D", optional=True),
        output_dataset=execution.output_file(f"{output_prefix}+tlrc.HEAD", optional=True),
    )
    execution.run(cargs)
    return ret
