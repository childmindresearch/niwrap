...
def afni_history(
    ...
    if check_date is not None:
        cargs.extend(["-check_date", check_date])
    if help_:
        cargs.append("-help")
    if history:
        cargs.append("-hist")
    ...
    if author is not None:
        cargs.extend(["-author", author])
    if level is not None:
        cargs.extend(["-level", str(level)])
    if min_level is not None:
        cargs.extend(["-min_level", str(min_level)])
    if program is not None:
        cargs.extend(["-program", program])
    if past_entries is not None:
        cargs.extend(["-past_entries", str(past_entries)])
    if past_days is not None:
        cargs.extend(["-past_days", str(past_days)])
    if past_months is not None:
        cargs.extend(["-past_months", str(past_months)])
    if past_years is not None:
        cargs.extend(["-past_years", str(past_years)])
    ...
    if show_field is not None:
        cargs.extend(["-show_field", show_field])
    if show_field_names:
