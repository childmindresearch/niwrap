...
def surf_clust(
    ...
    thresh_col: float | int | None = None,
    thresh: float | int | None = None,
    athresh: float | int | None = None,
    ir_range: list[float | int] | None = None,
    ex_range: list[float | int] | None = None,
    prepend_node_index: bool = False,
    update_: float | int | None = None,
    no_cent: bool = False,
    cent: bool = False,
    novolreg: bool = False,
    noxform: bool = False,
    ...
    if ir_range is not None and (len(ir_range) != 2): 
        raise ValueError(f"Length of 'ir_range' must be 2 but was {len(ir_range)}")
    if ex_range is not None and (len(ex_range) != 2): 
        raise ValueError(f"Length of 'ex_range' must be 2 but was {len(ex_range)}")
    ...
    if input_surface is not None:
        cargs.extend(["-surf_A", input_surface])
    if input_surf_name is not None:
        cargs.extend(["-i", execution.input_file(input_surf_name)])
    cargs.extend(["-input", *[execution.input_file(f) for f in input_dataset]])
    cargs.append("[DATA_COL_INDEX]")
    cargs.extend(["-rmm", str(rmm)])
    if amm2 is not None:
        cargs.extend(["-amm2", str(amm2)])
    if min_nodes is not None:
        cargs.extend(["-n", str(min_nodes)])
    ...
    if thresh_col is not None:
        cargs.extend(["-thresh_col", str(thresh_col)])
    if thresh is not None:
        cargs.extend(["-thresh", str(thresh)])
    if athresh is not None:
        cargs.extend(["-athresh", str(athresh)])
    if ir_range is not None:
        cargs.extend(["-ir_range", *map(str, ir_range)])
    if ex_range is not None:
        cargs.extend(["-ex_range", *map(str, ex_range)])
    if prepend_node_index:
        cargs.append("-prepend_node_index")
    if update_ is not None:
        cargs.extend(["-update", str(update_)])
    if no_cent:
    ...
    ret = SurfClustOutputs(
        ...
        cluster_table=execution.output_file(f"{prefix}_ClstTable_rXX_aXX.1D", optional=True) if prefix is not None else None,
        clustered_dataset=execution.output_file(f"{prefix}_Clustered_rXX_aXX.dset", optional=True) if prefix is not None else None,
        roi_dataset=execution.output_file(f"{prefix}_ClstMsk_rXX_aXX.dset", optional=True) if prefix is not None else None,
    )
    execution.run(cargs)
    return ret
