...
def v_3d_reg_ana(
    ...
    cargs.append("-rows")
    cargs.extend(["-rows", str(rows)])
    cargs.append("-cols")
    cargs.extend(["-cols", str(cols)])
    cargs.extend(["-xydata", *xydata])
    ...
    if rmsmin is not None:
        cargs.extend(["-rmsmin", str(rmsmin)])
    if fdisp is not None:
        cargs.extend(["-fdisp", str(fdisp)])
    if flof is not None:
        cargs.extend(["-flof", str(flof)])
    if fcoef is not None:
        cargs.extend(["-fcoef", *fcoef])
    if rcoef is not None:
        cargs.extend(["-rcoef", *rcoef])
    if tcoef is not None:
        cargs.extend(["-tcoef", *tcoef])
    if bucket is not None:
        cargs.extend(["-bucket", bucket])
    if brick is not None:
        cargs.extend(["-brick", *brick])
    if datum is not None:
        cargs.extend(["-datum", datum])
    ret = V3dRegAnaOutputs(
        root=execution.output_file("."),
        output_fift=execution.output_file(f"[PREFIX].fift+orig.HEAD", optional=True),
        output_fith=execution.output_file(f"[PREFIX].fith+orig.HEAD", optional=True),
        output_fitt=execution.output_file(f"[PREFIX].fitt+orig.HEAD", optional=True),
        output_bucket=execution.output_file(f"[PREFIX].bucket+orig.HEAD", optional=True),
        output_bucket_brik=execution.output_file(f"[PREFIX].bucket+orig.BRIK", optional=True),
    )
    execution.run(cargs)
    return ret
