...
def rba(
    ...
    if eoi is not None:
        cargs.extend(["-EOI", eoi])
    if wcp is not None:
        cargs.extend(["-WCP", str(wcp)])
    if tstat is not None:
        cargs.extend(["-tstat", tstat])
    if stdz is not None:
        cargs.extend(["-stdz", stdz])
    if c_vars is not None:
        cargs.extend(["-cVars", c_vars])
    if q_vars is not None:
        cargs.extend(["-qVars", q_vars])
    if dist_roi is not None:
        cargs.extend(["-distROI", dist_roi])
    if dist_subj is not None:
        cargs.extend(["-distSubj", dist_subj])
    if dist_y is not None:
        cargs.extend(["-distY", dist_y])
    if ridge_plot is not None:
        cargs.extend(["-ridgePlot", ridge_plot])
    if roi is not None:
        cargs.extend(["-ROI", roi])
    if subj is not None:
        cargs.extend(["-Subj", subj])
    if scale is not None:
        cargs.extend(["-scale", str(scale)])
    if se is not None:
        cargs.extend(["-se", se])
    if pdp is not None:
        cargs.extend(["-PDP", pdp])
    if mean is not None:
        cargs.extend(["-mean", mean])
    if sigma is not None:
        cargs.extend(["-sigma", sigma])
    if debug:
    ...
    ret = RbaOutputs(
        ...
        output_txt=execution.output_file(f"{prefix}.txt"),
        output_rdata=execution.output_file(f"{prefix}.RData", optional=True),
    )
    execution.run(cargs)
    return ret
