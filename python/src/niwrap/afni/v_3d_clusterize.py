...
def v_3d_clusterize(
    ...
    if out_mask is not None:
        cargs.extend(["-out_mask", out_mask])
    cargs.extend(["-ithr", ithr])
    if idat is not None:
        cargs.extend(["-idat", idat])
    if onesided is not None:
        cargs.extend(["-1sided", onesided])
    if twosided is not None:
        cargs.extend(["-2sided", twosided])
    if bisided is not None:
        cargs.extend(["-bisided", bisided])
    if within_range is not None:
        cargs.extend(["-within_range", within_range])
    cargs.extend(["-NN", str(nn)])
    if clust_nvox is not None:
        cargs.extend(["-clust_nvox", str(clust_nvox)])
    if clust_vol is not None:
        cargs.extend(["-clust_vol", str(clust_vol)])
    if pref_map is not None:
        cargs.extend(["-pref_map", pref_map])
    if pref_dat is not None:
        cargs.extend(["-pref_dat", pref_dat])
    if one_d_format:
    ...
    ret = V3dClusterizeOutputs(
        ...
        out_map_file=execution.output_file(f"{pref_map}+orig.HEAD", optional=True) if pref_map is not None else None,
        out_data_file=execution.output_file(f"{pref_dat}+orig.HEAD", optional=True) if pref_dat is not None else None,
        out_mask_file=execution.output_file(f"{out_mask}+orig.HEAD", optional=True) if out_mask is not None else None,
    )
    execution.run(cargs)
    return ret
