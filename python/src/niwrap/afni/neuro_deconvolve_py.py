...
def neuro_deconvolve_py(
    ...
    if kernel is not None:
        cargs.extend(["-kernel", kernel])
    if kernel_file is not None:
        cargs.extend(["-kernel_file", kernel_file])
    if mask_dset is not None:
        cargs.extend(["-mask_dset", execution.input_file(mask_dset)])
    if old_style:
        cargs.append("-old")
    if tr is not None:
        cargs.extend(["-tr", str(tr)])
    if tr_nup is not None:
        cargs.extend(["-tr_nup", str(tr_nup)])
    if verbosity is not None:
        cargs.extend(["-verb", str(verbosity)])
    ret = NeuroDeconvolvePyOutputs(
        root=execution.output_file("."),
        output_head=execution.output_file(f"{prefix}+orig.HEAD", optional=True),
        output_brik=execution.output_file(f"{prefix}+orig.BRIK", optional=True),
        kernel_file_out=execution.output_file(f"{kernel_file}", optional=True) if kernel_file is not None else None,
    )
    execution.run(cargs)
    return ret
