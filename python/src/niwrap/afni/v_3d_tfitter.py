...
def v_3d_tfitter(
    ...
    * At each voxel, assembles and solves a set of linear equations.
    ...
    ret = V3dTfitterOutputs(
        ...
        output_prefix=execution.output_file(f"{prefix}.nii.gz", optional=True) if prefix is not None else None,
        fitted_time_series=execution.output_file(f"{fitts}.nii.gz", optional=True) if fitts is not None else None,
        error_sums=execution.output_file(f"{errsum}.nii.gz", optional=True) if errsum is not None else None,
    )
    execution.run(cargs)
    return ret
