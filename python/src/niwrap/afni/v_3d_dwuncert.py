...
def v_3d_dwuncert(
    ...
    num_iters: float | int | None = None,
    mask_file: InputPathType | None = None,
    calc_thr_fa: float | int | None = None,
    ...
    if num_iters is not None:
        cargs.extend(["-iters", str(num_iters)])
    cargs.append("-mask")
    if mask_file is not None:
        cargs.extend(["-mask", execution.input_file(mask_file)])
    cargs.append("-calc_thr_FA")
    if calc_thr_fa is not None:
        cargs.extend(["-calc_thr_FA", str(calc_thr_fa)])
    cargs.append("-csf_fa")
    if csf_fa is not None:
        cargs.extend(["-csf_fa", str(csf_fa)])
    ret = V3dDwuncertOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{output_prefix}+*.HEAD/{output_prefix}+*.BRIK"),
    )
    execution.run(cargs)
    return ret
