...
def fat_proc_decmap(
    ...
    fa_thr: float | int | None = None,
    fa_sca: float | int | None = None,
    ...
    if fa_thr is not None:
        cargs.extend(["-fa_thr", str(fa_thr)])
    if fa_sca is not None:
        cargs.extend(["-fa_sca", str(fa_sca)])
    ...
    ret = FatProcDecmapOutputs(
        ...
        outfile_dec_rgb=execution.output_file(f"{prefix}_dec.nii.gz", optional=True),
        outfile_dec_unwt_thr=execution.output_file(f"{prefix}_dec_unwt_thr.nii.gz", optional=True),
        outfile_dec_sca=execution.output_file(f"{prefix}_dec_sca*.nii.gz", optional=True),
        qc_dec_images=execution.output_file(f"{prefix}_qc_dec*.png", optional=True),
        qc_dec_unwt_thrx_images=execution.output_file(f"{prefix}_qc_dec_unwt_thrx*.png", optional=True),
        qc_dec_sca_images=execution.output_file(f"{prefix}_qc_dec_sca*.png", optional=True),
    )
    execution.run(cargs)
    return ret
