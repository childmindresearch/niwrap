...
def gen_group_command(
    ...
    cargs.extend(["-command", command_name])
    cargs.extend(["-dsets", *datasets])
    cargs.append("[OPTIONS]")
    if prefix is not None:
        cargs.extend(["-prefix", prefix])
    if set_labels is not None:
        cargs.extend(["-set_labels", *set_labels])
    if subj_prefix is not None:
        cargs.extend(["-subj_prefix", subj_prefix])
    if subj_suffix is not None:
        cargs.extend(["-subj_suffix", subj_suffix])
    if subs_betas is not None:
        cargs.extend(["-subs_betas", *subs_betas])
    if subs_tstats is not None:
        cargs.extend(["-subs_tstats", *subs_tstats])
    if type_ is not None:
        cargs.extend(["-type", type_])
    if verb is not None:
        cargs.extend(["-verb", verb])
    if write_script is not None:
        cargs.extend(["-write_script", write_script])
    if other_options is not None:
        cargs.extend(["-options", *other_options])
    ret = GenGroupCommandOutputs(
        root=execution.output_file("."),
        output_script=execution.output_file(f"{write_script}", optional=True) if write_script is not None else None,
    )
    execution.run(cargs)
    return ret
