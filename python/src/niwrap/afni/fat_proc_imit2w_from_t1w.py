...
def fat_proc_imit2w_from_t1w(
    ...
    ss_blur_fwhm: float | int | None = None,
    no_clean: bool = False,
    ...
    cargs.extend(["-inset", execution.input_file(t1_file)])
    ...
    if ss_blur_fwhm is not None:
        cargs.extend(["-ss_blur_fwhm", str(ss_blur_fwhm)])
    if no_clean:
    ...
    ret = FatProcImit2wFromT1wOutputs(
        ...
        t2w_contrast_volume=execution.output_file(f"{prefix}.nii.gz", optional=True),
        cleaned_t1w_volume=execution.output_file(f"{prefix}_orig.nii.gz", optional=True),
        skull_stripped_t1w=execution.output_file(f"{prefix}_orig_ss.nii.gz", optional=True),
        qc_images=execution.output_file(f"{prefix}_qc*.nii.gz", optional=True),
    )
    execution.run(cargs)
    return ret
