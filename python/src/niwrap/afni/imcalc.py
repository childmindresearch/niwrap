...
def imcalc(
    ...
    if datum_type is not None:
        cargs.extend(["-datum type", datum_type])
    if image_inputs is not None:
        cargs.extend(["-a", *[execution.input_file(f) for f in image_inputs]])
    cargs.extend(["-expr", expression])
    if output_name is not None:
        cargs.extend(["-output", output_name])
    ret = ImcalcOutputs(
        root=execution.output_file("."),
        output_image=execution.output_file(f"{output_name}") if output_name is not None else None,
    )
    execution.run(cargs)
    return ret
