...
def v_3d_seg(
    ...
    if blur_meth is not None:
        cargs.extend(["-blur_meth", blur_meth])
    cargs.append("-bias_fwhm")
    if bias_fwhm is not None:
        cargs.extend(["-bias_fwhm", str(bias_fwhm)])
    cargs.append("-classes")
    if classes is not None:
        cargs.extend(["-classes", classes])
    cargs.append("-Bmrf")
    if bmrf is not None:
        cargs.extend(["-Bmrf", str(bmrf)])
    cargs.append("-bias_classes")
    if bias_classes is not None:
        cargs.extend(["-bias_classes", bias_classes])
    cargs.append("-prefix")
    ...
    if mixfrac is not None:
        cargs.extend(["-mixfrac", mixfrac])
    cargs.append("-mixfloor")
    if mixfloor is not None:
        cargs.extend(["-mixfloor", str(mixfloor)])
    cargs.append("-gold")
    if gold is not None:
        cargs.extend(["-gold", execution.input_file(gold)])
    cargs.append("-gold_bias")
    if gold_bias is not None:
        cargs.extend(["-gold_bias", execution.input_file(gold_bias)])
    cargs.append("-main_N")
    if main_n is not None:
        cargs.extend(["-main_N", str(main_n)])
    cargs.append("-cset")
    if cset is not None:
        cargs.extend(["-cset", execution.input_file(cset)])
    cargs.append("-labeltable")
    if labeltable is not None:
        cargs.extend(["-labeltable", execution.input_file(labeltable)])
    cargs.append("-vox_debug")
    if vox_debug is not None:
        cargs.extend(["-vox_debug", vox_debug])
    cargs.append("-vox_debug_file")
    if vox_debug_file is not None:
        cargs.extend(["-vox_debug_file", vox_debug_file])
    ret = V3dSegOutputs(
        root=execution.output_file("."),
        segmented_volume=execution.output_file(f"{prefix}_Segsy+orig.HEAD") if prefix is not None else None,
        bias_field=execution.output_file(f"{prefix}_BiasField+orig.HEAD", optional=True) if prefix is not None else None,
        classified_volume=execution.output_file(f"{prefix}_Classes+orig.HEAD", optional=True) if prefix is not None else None,
    )
    execution.run(cargs)
    return ret
