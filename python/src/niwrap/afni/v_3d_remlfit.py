...
def v_3d_remlfit(
    ...
    max_a_param: float | int | None = None,
    max_b_param: float | int | None = None,
    grid_param: float | int | None = None,
    ...
    cargs.extend(["-matrix", execution.input_file(regression_matrix)])
    if baseline_files is not None:
        cargs.extend(["-addbase", *baseline_files])
    if sort_nods:
    ...
    if output_prefix is not None:
        cargs.extend(["-Rvar", output_prefix])
    if go_for_it:
        cargs.append("-GOFORIT")
    if max_b_param is not None:
        cargs.extend(["-MAXb", str(max_b_param)])
    if grid_param is not None:
        cargs.extend(["-Grid", str(grid_param)])
    if negative_corr:
        cargs.append("-NEGcor")
    ret = V3dRemlfitOutputs(
        root=execution.output_file("."),
        outfile=execution.output_file(f"{output_prefix}.nii.gz", optional=True) if output_prefix is not None else None,
        rvar_file=execution.output_file(f"{output_prefix}_Rvar.nii.gz", optional=True) if output_prefix is not None else None,
        rbeta_file=execution.output_file(f"{output_prefix}_Rbeta.nii.gz", optional=True) if output_prefix is not None else None,
        rbuck_file=execution.output_file(f"{output_prefix}_Rbuck.nii.gz", optional=True) if output_prefix is not None else None,
        rfitts_file=execution.output_file(f"{output_prefix}_Rfitts.nii.gz", optional=True) if output_prefix is not None else None,
        rerrts_file=execution.output_file(f"{output_prefix}_Rerrts.nii.gz", optional=True) if output_prefix is not None else None,
    )
    execution.run(cargs)
    return ret
