...
class MakeLabelTableOutputs(typing.NamedTuple):
    """
    Output object returned when calling `_make_label_table(...)`.
    """
    root: OutputPathType
...
def _make_label_table(
    ...
    Returns:
        NamedTuple of outputs (described in `MakeLabelTableOutputs`).
    """
    ...
    execution = runner.start_execution(_MAKE_LABEL_TABLE_METADATA)
    cargs = []
    cargs.append("@MakeLabelTable")
    cargs.extend(["-labeltable", labeltable])
    if atlas_pointlist is not None:
        cargs.extend(["-atlas_pointlist", atlas_pointlist])
    if lab_r is not None:
        cargs.extend(["-lab_r", *lab_r])
    if lab_v is not None:
        cargs.extend(["-lab_v", *lab_v])
    if lab_file is not None:
        cargs.extend(["-lab_file", *lab_file])
    if dset is not None:
        cargs.extend(["-dset", execution.input_file(dset)])
    if longnames is not None:
        cargs.extend(["-longnames", str(longnames)])
    if last_longname_col is not None:
        cargs.extend(["-last_longname_col", str(last_longname_col)])
    if centers:
        cargs.append("-centers")
    if centertype is not None:
        cargs.extend(["-centertype", centertype])
    if centermask is not None:
        cargs.extend(["-centermask", centermask])
    if skip_novoxels:
    ...
    if lkeys is not None:
        cargs.extend(["-lkeys", lkeys])
    if rkeys is not None:
        cargs.extend(["-rkeys", rkeys])
    if klabel is not None:
        cargs.extend(["-klabel", klabel])
    if match_label is not None:
        cargs.extend(["-match_label", match_label])
    if labeltable_of_dset is not None:
        cargs.extend(["-labeltable_of_dset", execution.input_file(labeltable_of_dset)])
    if word_label_match:
        cargs.append("-word_label_match")
    if quiet_death:
        cargs.append("-quiet_death")
    if lt_to_atlas_pl is not None:
        cargs.extend(["-LT_to_atlas_PL", lt_to_atlas_pl])
    if dset_lt_to_atlas_pl is not None:
        cargs.extend(["-dset_LT_to_atlas_PL", *[execution.input_file(f) for f in dset_lt_to_atlas_pl]])
    if lt_to_csv is not None:
        cargs.extend(["-LT_to_CSV", execution.input_file(lt_to_csv)])
    if atlasize_labeled_dset is not None:
        cargs.extend(["-atlasize_labeled_dset", execution.input_file(atlasize_labeled_dset)])
    if atlas_file is not None:
        cargs.extend(["-atlas_file", atlas_file])
    if atlas_name is not None:
        cargs.extend(["-atlas_name", atlas_name])
    if atlas_description is not None:
        cargs.extend(["-atlas_description", atlas_description])
    if replace:
        cargs.append("-replace")
    if add_atlas_dset is not None:
        cargs.extend(["-add_atlas_dset", execution.input_file(add_atlas_dset)])
    if h_web:
        cargs.append("-h_web")
    if h_view:
        cargs.append("-h_view")
    ...
    if h_find is not None:
        cargs.extend(["-h_find", h_find])
    ret = MakeLabelTableOutputs(
        root=execution.output_file("."),
        output_labeltable=execution.output_file(f"{labeltable}.niml.lt"),
        output_atlas_pointlist=execution.output_file(f"{atlas_pointlist}.niml.atlas", optional=True) if atlas_pointlist is not None else None,
        output_csv=execution.output_file(f"{pathlib.Path(lt_to_csv).name}.csv", optional=True) if lt_to_csv is not None else None,
        output_niml_atlas=execution.output_file(f"{pathlib.Path(atlasize_labeled_dset).name}.niml", optional=True) if atlasize_labeled_dset is not None else None,
    )
    execution.run(cargs)
    return ret
