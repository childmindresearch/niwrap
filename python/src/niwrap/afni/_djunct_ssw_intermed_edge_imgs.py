...
class DjunctSswIntermedEdgeImgsOutputs(typing.NamedTuple):
    """
    Output object returned when calling `_djunct_ssw_intermed_edge_imgs(...)`.
    """
    root: OutputPathType
    """Output root folder. This is the root folder for all outputs."""


def _djunct_ssw_intermed_edge_imgs(
    prefix: str,
    ulay: InputPathType,
    olay: InputPathType,
    ...
    Returns:
        NamedTuple of outputs (described in `DjunctSswIntermedEdgeImgsOutputs`).
    """
    runner = runner or get_global_runner()
    execution = runner.start_execution(_DJUNCT_SSW_INTERMED_EDGE_IMGS_METADATA)
    cargs = []
    cargs.append("@djunct_ssw_intermed_edge_imgs")
    cargs.extend(["-prefix", prefix])
    cargs.extend(["-ulay", execution.input_file(ulay)])
    cargs.extend(["-olay", execution.input_file(olay)])
    if box_focus_slices is not None:
        cargs.extend(["-box_focus_slices", box_focus_slices])
    if montgap is not None:
        cargs.extend(["-montgap", montgap])
    if cbar is not None:
        cargs.extend(["-cbar", cbar])
    if ulay_range is not None:
        cargs.extend(["-ulay_range", ulay_range])
    if montx is not None:
        cargs.extend(["-montx", montx])
    if monty is not None:
        cargs.extend(["-monty", monty])
    if help_view:
        cargs.append("-hview")
    if help_:
        cargs.append("-help")
    ...
    ret = DjunctSswIntermedEdgeImgsOutputs(
...
__all__ = [
    "DjunctSswIntermedEdgeImgsOutputs",
    "_DJUNCT_SSW_INTERMED_EDGE_IMGS_METADATA",
    "_djunct_ssw_intermed_edge_imgs",
]
