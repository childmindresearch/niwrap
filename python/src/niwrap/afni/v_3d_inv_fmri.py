...
def v_3d_inv_fmri(
    ...
    polynom_order: float | int | None = None,
    ...
    cargs.extend(["-data", execution.input_file(input_file)])
    cargs.extend(["-map", execution.input_file(activation_map)])
    if map_weight is not None:
        cargs.extend(["-mapwt", execution.input_file(map_weight)])
    if mask is not None:
        cargs.extend(["-mask", execution.input_file(mask)])
    if baseline_file is not None:
        cargs.extend(["-base", *[execution.input_file(f) for f in baseline_file]])
    if polynom_order is not None:
        cargs.extend(["-polort", str(polynom_order)])
    ...
    ret = V3dInvFmriOutputs(
        ...
        outfile=execution.output_file(f"{output_file}", optional=True) if output_file is not None else None,
    )
    execution.run(cargs)
    return ret
