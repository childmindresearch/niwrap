...
def ap_run_simple_rest(
    ...
    ret = ApRunSimpleRestOutputs(
        ...
        run_ap_script=execution.output_file(f"run_ap_{subjid}", optional=True) if subjid is not None else None,
        proc_script=execution.output_file(f"proc.{subjid}", optional=True) if subjid is not None else None,
        proc_results_dir=execution.output_file(f"{subjid}.results", optional=True) if subjid is not None else None,
        text_output_files=execution.output_file(f"out.*", optional=True),
    )
    execution.run(cargs)
    return ret
