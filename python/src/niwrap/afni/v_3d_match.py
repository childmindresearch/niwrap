...
def v_3d_match(
    ...
    in_min: float | int | None = None,
    in_max: float | int | None = None,
    ref_min: float | int | None = None,
    ref_max: float | int | None = None,
    ...
    More information:
    https://afni.nimh.nih.gov/pub/dist/doc/program_help/3dMatch.html
    ...
    if in_min is not None:
        cargs.extend(["-in_min", str(in_min)])
    if in_max is not None:
        cargs.extend(["-in_max", str(in_max)])
    if ref_min is not None:
        cargs.extend(["-ref_min", str(ref_min)])
    if ref_max is not None:
        cargs.extend(["-ref_max", str(ref_max)])
    cargs.extend(["-prefix", prefix])
    if only_dice_thr:
        cargs.append("-only_dice_thr")
    ret = V3dMatchOutputs(
        root=execution.output_file("."),
        ref_brik=execution.output_file(f"{prefix}_REF+orig.BRIK"),
        ref_coeff_vals=execution.output_file(f"{prefix}_REF_coeff.vals"),
        in_brik=execution.output_file(f"{prefix}_IN+orig.BRIK"),
        in_coeff_vals=execution.output_file(f"{prefix}_IN_coeff.vals"),
    )
    execution.run(cargs)
    return ret
