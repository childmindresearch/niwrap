...
def fat_proc_axialize_anat(
    ...
    if qc_ulay_range is not None and (len(qc_ulay_range) != 2): 
        raise ValueError(f"Length of 'qc_ulay_range' must be 2 but was {len(qc_ulay_range)}")
    ...
    if extra_al_wtmask is not None:
        cargs.extend(["-extra_al_wtmask", execution.input_file(extra_al_wtmask)])
    if extra_al_cost is not None:
        cargs.extend(["-extra_al_cost", extra_al_cost])
    if extra_al_opts is not None:
        cargs.extend(["-extra_al_opts", extra_al_opts])
    if focus_mask is not None:
        cargs.extend(["-focus_mask", execution.input_file(focus_mask)])
    if focus_by_ss:
        cargs.append("-focus_by_ss")
    if remove_inf_sli is not None:
        cargs.extend(["-remove_inf_sli", str(remove_inf_sli)])
    ...
    ret = FatProcAxializeAnatOutputs(
        ...
        working_directory=execution.output_file(f"{workdir}", optional=True) if workdir is not None else None,
    )
    execution.run(cargs)
    return ret
