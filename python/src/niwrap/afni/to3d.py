...
def to3d(
    ...
    statpar: list[float | int] | None = None,
    ...
    zorigin: float | int | None = None,
    data_type: typing.Literal["short", "float", "byte", "complex"] | None = None,
    global_scaling_factor: float | int | None = None,
    ...
    if statpar is not None:
        cargs.extend(["-statpar", *map(str, statpar)])
    if prefix is not None:
        cargs.extend(["-prefix", prefix])
    if session is not None:
        cargs.extend(["-session", session])
    if geomparent is not None:
        cargs.extend(["-geomparent", execution.input_file(geomparent)])
    if anatparent is not None:
        cargs.extend(["-anatparent", execution.input_file(anatparent)])
    if nosave_flag:
        cargs.append("-nosave")
    if nowritebrik_flag:
        cargs.append("-nowritebrik")
    if view is not None:
        cargs.extend(["-view", view])
    if time_zt is not None:
        cargs.extend(["-time:zt", *time_zt])
    if time_tz is not None:
        cargs.extend(["-time:tz", *time_tz])
    if tr_units is not None:
        cargs.extend(["-t", tr_units])
    if torg is not None:
        cargs.extend(["-Torg", str(torg)])
    if x_fov is not None:
        cargs.extend(["-xFOV", x_fov])
    if y_fov is not None:
        cargs.extend(["-yFOV", y_fov])
    if z_fov is not None:
        cargs.extend(["-zFOV", z_fov])
    if x_slab is not None:
        cargs.extend(["-xSLAB", x_slab])
    if y_slab is not None:
        cargs.extend(["-ySLAB", y_slab])
    if z_slab is not None:
        cargs.extend(["-zSLAB", z_slab])
    if zorigin is not None:
        cargs.extend(["-zorigin", str(zorigin)])
    if data_type is not None:
        cargs.extend(["-datum", data_type])
    if global_scaling_factor is not None:
        cargs.extend(["-gsfac", str(global_scaling_factor)])
    if nofloatscan_flag:
    ...
    if save_outliers is not None:
        cargs.extend(["-save_outliers", save_outliers])
    ...
    if ncolors is not None:
        cargs.extend(["-ncolors", str(ncolors)])
    if xtwarns_flag:
    ...
    ret = To3dOutputs(
        ...
        headfile=execution.output_file(f"{prefix}.HEAD") if prefix is not None else None,
        brikfile=execution.output_file(f"{prefix}.BRIK") if prefix is not None else None,
        outfile_outliers=execution.output_file(f"{save_outliers}", optional=True) if save_outliers is not None else None,
    )
    execution.run(cargs)
    return ret
