...
class AutoWarpOutputs(typing.NamedTuple):
    """
    Output object returned when calling `auto_warp(...)`.
    """
    root: OutputPathType
...
def auto_warp(
    ...
    warp_dxyz: float | int | None = None,
    affine_dxyz: float | int | None = None,
    ...
    Returns:
        NamedTuple of outputs (described in `AutoWarpOutputs`).
    """
    runner = runner or get_global_runner()
    execution = runner.start_execution(AUTO_WARP_METADATA)
    cargs = []
    ...
    if qblur is not None:
        cargs.extend(["-qblur", qblur])
    if qworkhard is not None:
        cargs.extend(["-qworkhard", qworkhard])
    if qw_opts is not None:
        cargs.extend(["-qw_opts", qw_opts])
    if keep_rm_files:
    ...
    if ex_mode is not None:
        cargs.extend(["-ex_mode", ex_mode])
    if overwrite:
        cargs.append("-overwrite")
    if suffix is not None:
        cargs.extend(["-suffix", suffix])
    if child_anat is not None:
        cargs.extend(["-child_anat", child_anat])
    if warp_dxyz is not None:
        cargs.extend(["-warp_dxyz", str(warp_dxyz)])
    if affine_dxyz is not None:
        cargs.extend(["-affine_dxyz", str(affine_dxyz)])
    if affine_input_xmat is not None:
        cargs.extend(["-affine_input_xmat", affine_input_xmat])
    if smooth_anat:
    ...
    if output_dir is not None:
        cargs.extend(["-output_dir", output_dir])
    if followers is not None:
        cargs.extend(["-followers", followers])
    if affine_followers_xmat is not None:
        cargs.extend(["-affine_followers_xmat", affine_followers_xmat])
    if skullstrip_opts is not None:
        cargs.extend(["-skullstrip_opts", skullstrip_opts])
    if at_opts is not None:
        cargs.extend(["-at_opts", at_opts])
    ret = AutoWarpOutputs(
