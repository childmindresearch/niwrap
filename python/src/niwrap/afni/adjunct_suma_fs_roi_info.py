...
def adjunct_suma_fs_roi_info(
    ...
    cargs.extend(["-suma_dir", suma_directory])
    if help_:
        cargs.append("-help")
    if hview:
        cargs.append("-hview")
    ...
    ret = AdjunctSumaFsRoiInfoOutputs(
        ...
        rois_2000_ft=execution.output_file(f"stats_fs_rois_2000_FT.1D", optional=True),
        rois_2009_ft=execution.output_file(f"stats_fs_rois_2009_FT.1D", optional=True),
        segs_2000_ft=execution.output_file(f"stats_fs_segs_2000_FT.1D", optional=True),
        segs_2009_ft=execution.output_file(f"stats_fs_segs_2009_FT.1D", optional=True),
    )
    execution.run(cargs)
    return ret
