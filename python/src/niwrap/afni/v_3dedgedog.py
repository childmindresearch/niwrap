...
def v_3dedgedog(
    ...
    sigma_rad: float | int | None = None,
    sigma_nvox: float | int | None = None,
    ratio_sigma: float | int | None = None,
    output_intermed: bool = False,
    edge_bnd_nn: float | int | None = None,
    ...
    if automask is not None:
        cargs.extend(["-automask", automask])
    if sigma_rad is not None:
        cargs.extend(["-sigma_rad", str(sigma_rad)])
    if sigma_nvox is not None:
        cargs.extend(["-sigma_nvox", str(sigma_nvox)])
    if ratio_sigma is not None:
        cargs.extend(["-ratio_sigma", str(ratio_sigma)])
    if output_intermed:
        cargs.append("-output_intermed")
    if edge_bnd_nn is not None:
        cargs.extend(["-edge_bnd_NN", str(edge_bnd_nn)])
    if edge_bnd_side is not None:
        cargs.extend(["-edge_bnd_side", edge_bnd_side])
    if edge_bnd_scale:
        cargs.append("-edge_bnd_scale")
    if only2d is not None:
        cargs.extend(["-only2D", only2d])
    ret = V3dedgedogOutputs(
        root=execution.output_file("."),
        out_edge=execution.output_file(f"{prefix}_edge.nii.gz"),
        out_dog=execution.output_file(f"{prefix}_dog.nii.gz", optional=True),
        out_edt2=execution.output_file(f"{prefix}_edt2.nii.gz", optional=True),
        out_blur_inner=execution.output_file(f"{prefix}_blur_inner.nii.gz", optional=True),
        out_blur_outer=execution.output_file(f"{prefix}_blur_outer.nii.gz", optional=True),
    )
    execution.run(cargs)
    return ret
