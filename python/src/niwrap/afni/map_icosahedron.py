...
def map_icosahedron(
    ...
    if rec_depth is not None:
        cargs.extend(["-rd", str(rec_depth)])
    if lin_depth is not None:
        cargs.extend(["-ld", str(lin_depth)])
    if morph_surf is not None:
        cargs.extend(["-morph", morph_surf])
    if num_it is not None:
        cargs.extend(["-it", str(num_it)])
    if prefix is not None:
        cargs.extend(["-prefix", prefix])
    if nn_dset is not None:
        cargs.extend(["-NN_dset_map", nn_dset])
    if dset is not None:
        cargs.extend(["-dset_map", dset])
    if fix_cut_surfaces:
