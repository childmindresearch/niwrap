...
def realtime_receiver(
    ...
    if show_data is not None:
        cargs.extend(["-show_data", show_data])
    if write_text_data is not None:
        cargs.extend(["-write_text_data", write_text_data])
    if data_choice is not None:
        cargs.extend(["-data_choice", data_choice])
    if serial_port is not None:
        cargs.extend(["-serial_port", serial_port])
    if show_demo_gui is not None:
        cargs.extend(["-show_demo_gui", show_demo_gui])
    if dc_params is not None:
        cargs.extend(["-dc_params", *map(str, dc_params)])
    if extras_on_one_line is not None:
        cargs.extend(["-extras_on_one_line", extras_on_one_line])
    if show_comm_times:
    ...
    if tcp_port is not None:
        cargs.extend(["-tcp_port", str(tcp_port)])
