...
class AnimalWarperOutputs(typing.NamedTuple):
    """
    Output object returned when calling `_animal_warper(...)`.
    """
    root: OutputPathType
...
def _animal_warper(
    ...
    feature_size: float | int | None = None,
    supersize: bool = False,
    init_scale: float | int | None = None,
    mode_smooth_size: float | int | None = None,
    ...
    Returns:
        NamedTuple of outputs (described in `AnimalWarperOutputs`).
    """
    runner = runner or get_global_runner()
    if maxlev is not None and not (0 <= maxlev <= 11): 
        raise ValueError(f"'maxlev' must be between 0 <= x <= 11 but was {maxlev}")
    execution = runner.start_execution(_ANIMAL_WARPER_METADATA)
    cargs = []
    ...
    if atlases is not None:
        cargs.extend(["-atlas", *[execution.input_file(f) for f in atlases]])
    if atlas_followers is not None:
        cargs.extend(["-atlas_followers", *[execution.input_file(f) for f in atlas_followers]])
    if seg_followers is not None:
        cargs.extend(["-seg_followers", *[execution.input_file(f) for f in seg_followers]])
    if template_followers is not None:
        cargs.extend(["-template_followers", *[execution.input_file(f) for f in template_followers]])
    if dset_followers is not None:
        cargs.extend(["-dset_followers", *[execution.input_file(f) for f in dset_followers]])
    if roidset_followers is not None:
        cargs.extend(["-roidset_followers", *[execution.input_file(f) for f in roidset_followers]])
    if input_abbrev is not None:
        cargs.extend(["-input_abbrev", input_abbrev])
    if base_abbrev is not None:
        cargs.extend(["-base_abbrev", base_abbrev])
    if atlas_abbrevs is not None:
        cargs.extend(["-atlas_abbrevs", *atlas_abbrevs])
    if template_abbrevs is not None:
        cargs.extend(["-template_abbrevs", *template_abbrevs])
    if seg_abbrevs is not None:
        cargs.extend(["-seg_abbrevs", *seg_abbrevs])
    if dset_abbrevs is not None:
        cargs.extend(["-dset_abbrevs", *dset_abbrevs])
    if roidset_abbrevs is not None:
        cargs.extend(["-roidset_abbrevs", *roidset_abbrevs])
    if align_centers_meth is not None:
        cargs.extend(["-align_centers_meth", align_centers_meth])
    if aff_move_opt is not None:
        cargs.extend(["-aff_move_opt", aff_move_opt])
    if cost is not None:
        cargs.extend(["-cost", cost])
    if maxlev is not None:
        cargs.extend(["-maxlev", str(maxlev)])
    if no_surfaces:
        cargs.append("-no_surfaces")
    if feature_size is not None:
        cargs.extend(["-feature_size", str(feature_size)])
    if supersize:
        cargs.append("-supersize")
    if init_scale is not None:
        cargs.extend(["-init_scale", str(init_scale)])
    if mode_smooth_size is not None:
        cargs.extend(["-mode_smooth_size", str(mode_smooth_size)])
    if mode_smooth_replacement_off:
        cargs.append("-mode_smooth_replacement_off")
    if center_out is not None:
        cargs.extend(["-center_out", center_out])
    if align_type is not None:
        cargs.extend(["-align_type", align_type])
    if extra_qw_opts is not None:
        cargs.extend(["-extra_qw_opts", extra_qw_opts])
    if keep_temp:
    ...
    if echo:
        cargs.append("-echo")
    ret = AnimalWarperOutputs(
        root=execution.output_file("."),
        warp2std=execution.output_file(f"aw_results/{pathlib.Path(input_file).name}_warp2std.nii.gz", optional=True),
        qc_image_initial=execution.output_file(f"aw_results/init_qc_00.input+base*.jpg", optional=True),
        qc_image_initial_sh=execution.output_file(f"aw_results/init_qc_01.input_sh+base*.jpg", optional=True),
        dset_followers_out=execution.output_file(f"aw_results/DSET_FOLL.nii.gz", optional=True),
        roidset_followers_out=execution.output_file(f"aw_results/ROIDSET_FOLL.nii.gz", optional=True),
        animal_outs_guide=execution.output_file(f"aw_results/animal_outs.txt", optional=True),
    )
    execution.run(cargs)
    return ret
