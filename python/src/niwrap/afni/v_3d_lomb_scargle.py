...
def v_3d_lomb_scargle(
    ...
    More information:
    https://afni.nimh.nih.gov/pub/dist/doc/program_help/3dLombScargle.html
    ...
    if censor_1d is not None:
        cargs.extend(["-censor_1D", execution.input_file(censor_1d)])
    if censor_string is not None:
        cargs.extend(["-censor_str", censor_string])
    if mask_file is not None:
        cargs.extend(["-mask", execution.input_file(mask_file)])
    if out_pow_spec:
        cargs.append("-out_pow_spec")
    if nyquist_multiplier is not None:
        cargs.extend(["-nyq_mult", str(nyquist_multiplier)])
    if nifti:
        cargs.append("-nifti")
    ret = V3dLombScargleOutputs(
        root=execution.output_file("."),
        time_points=execution.output_file(f"{prefix}_time.1D"),
        frequency_points=execution.output_file(f"{prefix}_freq.1D"),
        amplitude_spectrum=execution.output_file(f"{prefix}_amp+orig", optional=True),
        power_spectrum=execution.output_file(f"{prefix}_pow+orig", optional=True),
    )
    execution.run(cargs)
    return ret
