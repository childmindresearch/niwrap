...
class UpdateAfniBinariesOutputs(typing.NamedTuple):
    """
    Output object returned when calling `update_afni_binaries(...)`.
    """
    root: OutputPathType
...
def update_afni_binaries(
    ...
    Returns:
        NamedTuple of outputs (described in `UpdateAfniBinariesOutputs`).
    """
    ...
    if apsearch is not None:
        cargs.extend(["-apsearch", apsearch])
    if bindir is not None:
        cargs.extend(["-bindir", bindir])
    if curl_flag:
        cargs.append("-curl")
    if do_dotfiles_flag:
    ...
    if local_package is not None:
        cargs.extend(["-local_package", local_package])
    if prog_list is not None:
        cargs.extend(["-prog_list", *prog_list])
    if package is not None:
        cargs.extend(["-package", package])
    ret = UpdateAfniBinariesOutputs(
