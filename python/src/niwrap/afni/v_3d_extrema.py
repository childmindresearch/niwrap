...
def v_3d_extrema(
    ...
    mask_threshold: float | int | None = None,
    data_threshold: float | int | None = None,
    n_best: float | int | None = None,
    separation_distance: float | int | None = None,
    minima: bool = False,
    maxima: bool = False,
    strict: bool = False,
    partial: bool = False,
    ...
    if output_session is not None:
        cargs.extend(["-session", output_session])
    if quiet:
        cargs.append("-quiet")
    if mask_file is not None:
        cargs.extend(["-mask_file", execution.input_file(mask_file)])
    if mask_threshold is not None:
        cargs.extend(["-mask_thr", str(mask_threshold)])
    if data_threshold is not None:
        cargs.extend(["-data_thr", str(data_threshold)])
    if n_best is not None:
        cargs.extend(["-nbest", str(n_best)])
    if separation_distance is not None:
        cargs.extend(["-sep_dist", str(separation_distance)])
    if minima:
        cargs.append("-minima")
    if maxima:
        cargs.append("-maxima")
    ...
    ret = V3dExtremaOutputs(
        ...
        output_head_file=execution.output_file(f"{output_prefix}.HEAD", optional=True) if output_prefix is not None else None,
        output_brik_file=execution.output_file(f"{output_prefix}.BRIK", optional=True) if output_prefix is not None else None,
    )
    execution.run(cargs)
    return ret
