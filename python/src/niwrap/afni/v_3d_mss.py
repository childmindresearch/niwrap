...
def v_3d_mss(
    ...
    if mrr_formula is not None:
        cargs.extend(["-mrr", mrr_formula])
    if lme_formula is not None:
        cargs.extend(["-lme", lme_formula])
    if random_effect is not None:
        cargs.extend(["-ranEff", random_effect])
    if qvars is not None:
        cargs.extend(["-qVars", qvars])
    if mask is not None:
        cargs.extend(["-mask", execution.input_file(mask)])
    if bounds is not None:
        cargs.extend(["-bounds", *map(str, bounds)])
    ...
    if if_name is not None:
        cargs.extend(["-IF", if_name])
    if show_allowed_options_flag:
        cargs.append("-show_allowed_options")
    if sdiff_vars is not None:
        cargs.extend(["-sdiff", sdiff_vars])
    if vt_formula is not None:
        cargs.extend(["-vt", vt_formula])
    ret = V3dMssOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{prefix}.nii", optional=True),
    )
    execution.run(cargs)
    return ret
