...
def v_1d_marry(
    ...
    ret = V1dMarryOutputs(
        ...
        outfile=execution.output_file(f"stdout"),
        divorcee_a=execution.output_file(f"[FILE2]_A.1D", optional=True),
        divorcee_b=execution.output_file(f"[FILE2]_B.1D", optional=True),
    )
    execution.run(cargs)
    return ret
