...
def v_3d_mean(
    ...
    if weightset is not None:
        cargs.extend(["-weightset", execution.input_file(weightset)])
    ret = V3dMeanOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{prefix}<+optional_extension>", optional=True) if prefix is not None else None,
    )
    execution.run(cargs)
    return ret
