...
def surf_patch(
    ...
    coord_gain: float | int | None = None,
    ...
    cargs.extend(["-surf_B", execution.input_file(surf_b)])
    cargs.append("-input")
    cargs.extend(["-input", execution.input_file(nodefile)])
    ...
    ret = SurfPatchOutputs(
        ...
        outpatch_a=execution.output_file(f"{prefix}_A", optional=True),
        outpatch_b=execution.output_file(f"{prefix}_B", optional=True),
        out_stitched_surface=execution.output_file(f"{prefix}_stitched", optional=True),
    )
    execution.run(cargs)
    return ret
