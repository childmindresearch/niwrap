...
def v_1d_nlfit(
    ...
    cargs.extend(["-indvar", independent_variable])
    cargs.extend(["-param", *parameters])
    cargs.extend(["-depdata", execution.input_file(dependent_data)])
    if method is not None:
        cargs.extend(["-meth", str(method)])
    ret = V1dNlfitOutputs(
        root=execution.output_file("."),
        fit_results=execution.output_file(f"stdout"),
    )
    execution.run(cargs)
    return ret
