...
def trr(
    ...
    model: str | None = None,
    plot_size: list[float | int] | None = None,
    standard_error: str | None = None,
    t_stat: str | None = None,
    within_chain_parallelization: float | int | None = None,
    debug: bool = False,
    ...
    if plot_size is not None and (len(plot_size) != 2): 
        raise ValueError(f"Length of 'plot_size' must be 2 but was {len(plot_size)}")
    execution = runner.start_execution(TRR_METADATA)
    cargs = []
    cargs.append("TRR")
    cargs.extend(["-prefix", prefix])
    if chains is not None:
        cargs.extend(["-chains", str(chains)])
    if iterations is not None:
        cargs.extend(["-iterations", str(iterations)])
    cargs.extend(["-Y", response_var])
    cargs.extend(["-subject", subject_var])
    if repetition_var is not None:
        cargs.extend(["-repetition", repetition_var])
    if condition_var is not None:
        cargs.extend(["-condition", condition_var])
    cargs.extend(["-dataTable", execution.input_file(data_table)])
    if categorical_vars is not None:
        cargs.extend(["-cVars", categorical_vars])
    if quantitative_vars is not None:
        cargs.extend(["-qVars", quantitative_vars])
    if response_dist is not None:
        cargs.extend(["-distY", response_dist])
    if model is not None:
        cargs.extend(["-model", model])
    if plot_size is not None:
        cargs.extend(["-PDP", *map(str, plot_size)])
    if standard_error is not None:
        cargs.extend(["-se", standard_error])
    if t_stat is not None:
        cargs.extend(["-tstat", t_stat])
    if within_chain_parallelization is not None:
        cargs.extend(["-WCP", str(within_chain_parallelization)])
    if debug:
    ...
    ret = TrrOutputs(
        ...
        output_file_txt=execution.output_file(f"{prefix}.txt"),
        output_file_pdf=execution.output_file(f"{prefix}.pdf", optional=True),
        output_file_rdata=execution.output_file(f"{prefix}.RData", optional=True),
    )
    execution.run(cargs)
    return ret
