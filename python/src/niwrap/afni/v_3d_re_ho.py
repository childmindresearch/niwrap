...
def v_3d_re_ho(
    ...
    neigh_rad: float | int | None = None,
    neigh_x: float | int | None = None,
    neigh_y: float | int | None = None,
    neigh_z: float | int | None = None,
    box_rad: float | int | None = None,
    box_x: float | int | None = None,
    box_y: float | int | None = None,
    box_z: float | int | None = None,
    ...
    if nneigh is not None:
        cargs.extend(["-nneigh", nneigh])
    if chi_sq:
        cargs.append("-chi_sq")
    if mask is not None:
        cargs.extend(["-mask", execution.input_file(mask)])
    if neigh_rad is not None:
        cargs.extend(["-neigh_RAD", str(neigh_rad)])
    if neigh_x is not None:
        cargs.extend(["-neigh_X", str(neigh_x)])
    if neigh_y is not None:
        cargs.extend(["-neigh_Y", str(neigh_y)])
    if neigh_z is not None:
        cargs.extend(["-neigh_Z", str(neigh_z)])
    if box_rad is not None:
        cargs.extend(["-box_RAD", str(box_rad)])
    if box_x is not None:
        cargs.extend(["-box_X", str(box_x)])
    if box_y is not None:
        cargs.extend(["-box_Y", str(box_y)])
    if box_z is not None:
        cargs.extend(["-box_Z", str(box_z)])
    if in_rois is not None:
        cargs.extend(["-in_rois", execution.input_file(in_rois)])
    ret = V3dReHoOutputs(
        root=execution.output_file("."),
        reho_output=execution.output_file(f"{prefix}+orig.BRIK"),
        roi_reho_vals=execution.output_file(f"{prefix}_ROI_reho.vals", optional=True),
        chi_square=execution.output_file(f"{prefix}+orig.BRIK[1]", optional=True),
        roi_chi_vals=execution.output_file(f"{prefix}_ROI_reho.chi", optional=True),
    )
    execution.run(cargs)
    return ret
