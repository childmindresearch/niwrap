...
def cifti_tool(
    ...
    verbose_read_level: float | int | None = None,
    both_verbose_levels: float | int | None = None,
    ...
    if eval_type is not None:
        cargs.extend(["-eval_type", eval_type])
    cargs.append("-output")
    ...
    if verbose_read_level is not None:
        cargs.extend(["-verb_read", str(verbose_read_level)])
    if both_verbose_levels is not None:
        cargs.extend(["-vboth", str(both_verbose_levels)])
    ret = CiftiToolOutputs(
        root=execution.output_file("."),
        output_file=execution.output_file(f"{output_file}", optional=True) if output_file is not None else None,
    )
    execution.run(cargs)
    return ret
