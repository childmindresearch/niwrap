...
def v_3d_afnito_analyze(
    ...
    if orient_option is not None:
        cargs.extend(["-orient", orient_option])
    cargs.append(output_name)
    cargs.append(execution.input_file(afni_dataset))
    ret = V3dAfnitoAnalyzeOutputs(
        root=execution.output_file("."),
        output_hdr_file=execution.output_file(f"{output_name}_[INDEX].hdr"),
        output_img_file=execution.output_file(f"{output_name}_[INDEX].img"),
        output_4d_hdr_file=execution.output_file(f"{output_name}.hdr", optional=True),
        output_4d_img_file=execution.output_file(f"{output_name}.img", optional=True),
    )
    execution.run(cargs)
    return ret
