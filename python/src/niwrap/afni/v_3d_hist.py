...
def v_3d_hist(
    ...
    dind_subbrick: float | int | None = None,
    ...
    binwidth: float | int | None = None,
    ignore_out: bool = False,
    range_hist: InputPathType | None = None,
    showhist: bool = False,
    at_val: float | int | None = None,
    get_params: str | None = None,
    voxvol: float | int | None = None,
    ...
    if mask_dset is not None:
        cargs.extend(["-mask", execution.input_file(mask_dset)])
    if mask_range is not None:
        cargs.extend(["-mask_range", *map(str, mask_range)])
    if cmask is not None:
        cargs.extend(["-cmask", cmask])
    if hist_file is not None:
        cargs.extend(["-thishist", execution.input_file(hist_file)])
    if prefix is not None:
        cargs.extend(["-prefix", prefix])
    if equalized is not None:
        cargs.extend(["-equalized", equalized])
    if nbin is not None:
        cargs.extend(["-nbin", str(nbin)])
    if min_ is not None:
        cargs.extend(["-min", str(min_)])
    if max_ is not None:
        cargs.extend(["-max", str(max_)])
    if binwidth is not None:
        cargs.extend(["-binwidth", str(binwidth)])
    if ignore_out:
        cargs.append("-ignore_out")
    if range_hist is not None:
        cargs.extend(["-rhist", execution.input_file(range_hist)])
    if showhist:
        cargs.append("-showhist")
    if at_val is not None:
        cargs.extend(["-at", str(at_val)])
    if get_params is not None:
        cargs.extend(["-get", get_params])
    if voxvol is not None:
        cargs.extend(["-voxvol", str(voxvol)])
    if val_at is not None:
        cargs.extend(["-val_at", val_at])
    if quiet:
        cargs.append("-quiet")
    ret = V3dHistOutputs(
