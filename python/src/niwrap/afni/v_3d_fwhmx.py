...
def v_3d_fwhmx(
    ...
    if compat:
        cargs.append("-compat")
    if acf is not None:
        cargs.extend(["-acf", acf])
    cargs.append(execution.input_file(infile))
    ret = V3dFwhmxOutputs(
        root=execution.output_file("."),
        out_file=execution.output_file(f"{out}.1D", optional=True) if out is not None else None,
        detrended_dataset=execution.output_file(f"{detprefix}.nii.gz", optional=True) if detprefix is not None else None,
    )
    execution.run(cargs)
    return ret
