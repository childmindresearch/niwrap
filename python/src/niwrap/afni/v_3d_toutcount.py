...
def v_3d_toutcount(
    ...
    fraction: bool = False,
    range_: bool = False,
    polort_order: float | int | None = None,
    legendre: bool = False,
    ...
    if mask_dataset is not None:
        cargs.extend(["-mask", mask_dataset])
    if q_threshold is not None:
        cargs.extend(["-qthr", str(q_threshold)])
    if autoclip:
    ...
    if polort_order is not None:
        cargs.extend(["-polort", str(polort_order)])
    if legendre:
        cargs.append("-legendre")
    ret = V3dToutcountOutputs(
        root=execution.output_file("."),
        output_afni_head=execution.output_file(f"{output_prefix}.HEAD", optional=True) if output_prefix is not None else None,
        output_afni_brik=execution.output_file(f"{output_prefix}.BRIK", optional=True) if output_prefix is not None else None,
    )
    execution.run(cargs)
    return ret
