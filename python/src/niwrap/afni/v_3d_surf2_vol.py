...
def v_3d_surf2_vol(
    ...
    cargs.extend(["-spec", execution.input_file(spec)])
    ...
    if surf_xyz_1d is not None:
        cargs.extend(["-surf_xyz_1D", execution.input_file(surf_xyz_1d)])
    if sdata_1d is not None:
        cargs.extend(["-sdata_1D", execution.input_file(sdata_1d)])
    if sdata is not None:
        cargs.extend(["-sdata", execution.input_file(sdata)])
    ...
    if data_expr is not None:
        cargs.extend(["-data_expr", data_expr])
    ...
    if dvoxel is not None:
        cargs.extend(["-dvoxel", str(dvoxel)])
    if noscale:
    ...
    ret = V3dSurf2VolOutputs(
        ...
        output_file=execution.output_file(f"{prefix}+*[gz]"),
    )
    execution.run(cargs)
    return ret
