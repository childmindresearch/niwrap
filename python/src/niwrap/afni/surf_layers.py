...
def surf_layers(
    ...
    hemi: str | None = None,
    n_intermed_surfs: float | int | None = None,
    ...
    if spec_dset is not None:
        cargs.extend(["-spec", execution.input_file(spec_dset)])
    if outdir is not None:
        cargs.extend(["-outdir", outdir])
    if states is not None:
        cargs.extend(["-states", states])
    if hemi is not None:
        cargs.extend(["-hemi", hemi])
    if n_intermed_surfs is not None:
        cargs.extend(["-n_intermed_surfs", str(n_intermed_surfs)])
    if surf_a is not None:
        cargs.extend(["-surf_A", execution.input_file(surf_a)])
    if surf_b is not None:
        cargs.extend(["-surf_B", execution.input_file(surf_b)])
    ...
    ret = SurfLayersOutputs(
        ...
        interpolated_surfaces=execution.output_file(f"{outdir}/isurf.{hemi}.*.gii", optional=True) if outdir is not None and hemi is not None else None,
        additional_spec_files=execution.output_file(f"{outdir}/*", optional=True) if outdir is not None else None,
        run_view_script=execution.output_file(f"{outdir}/run*tcsh", optional=True) if outdir is not None else None,
    )
    execution.run(cargs)
    return ret
