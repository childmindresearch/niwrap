...
def v_3d_gen_priors(
    ...
    if uid is not None:
        cargs.extend(["-uid", uid])
    if use_tmp:
        cargs.append("-use_tmp")
    if no_tmp:
        cargs.append("-no_tmp")
    if pset is not None:
        cargs.extend(["-pset", pset])
    if cset is not None:
        cargs.extend(["-cset", cset])
    if regroup_classes is not None:
        cargs.extend(["-regroup_classes", regroup_classes])
    if classes is not None:
        cargs.extend(["-classes", classes])
    if features is not None:
        cargs.extend(["-features", features])
    if strict_feature_match:
        cargs.append("-strict_feature_match")
    if featgroups is not None:
        cargs.extend(["-featgroups", featgroups])
    if show_this_dist is not None:
        cargs.extend(["-ShowThisDist", show_this_dist])
    if fast:
        cargs.append("-fast")
    if slow:
        cargs.append("-slow")
    ret = V3dGenPriorsOutputs(
        ...
        out_cprefix=execution.output_file(f"{cprefix}.nii.gz", optional=True),
        out_pprefix=execution.output_file(f"{pprefix}.nii.gz", optional=True),
    )
    execution.run(cargs)
    return ret
