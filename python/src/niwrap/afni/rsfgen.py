...
def rsfgen(
    ...
    permutation_seed: float | int | None = None,
    markov_file: InputPathType | None = None,
    prob_zero: float | int | None = None,
    ...
    cargs = []
    cargs.append("RSFgen")
    cargs.extend(["-nt", str(length)])
    cargs.extend(["-num_stimts", str(num_experimental_conditions)])
    if block_length is not None:
        cargs.extend(["-nblock", block_length])
    if random_seed is not None:
        cargs.extend(["-seed", str(random_seed)])
    ...
    if num_reps is not None:
        cargs.extend(["-nreps", num_reps])
    if permutation_seed is not None:
        cargs.extend(["-pseed", str(permutation_seed)])
    if markov_file is not None:
        cargs.extend(["-markov", execution.input_file(markov_file)])
    if prob_zero is not None:
        cargs.extend(["-pzero", str(prob_zero)])
    if input_table is not None:
        cargs.extend(["-table", execution.input_file(input_table)])
    ret = RsfgenOutputs(
        root=execution.output_file("."),
        output_files=execution.output_file(f"{output_prefix}1.1D") if output_prefix is not None else None,
    )
    execution.run(cargs)
    return ret
