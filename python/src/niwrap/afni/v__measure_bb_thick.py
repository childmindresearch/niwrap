...
class MeasureBbThickOutputs(typing.NamedTuple):
    """
    Output object returned when calling `measure_bb_thick(...)`.
    """
    root: OutputPathType
...
def measure_bb_thick(
    ...
    depth_search: float | int | None = None,
    ...
    Returns:
        NamedTuple of outputs (described in `MeasureBbThickOutputs`).
    """
    ...
    if depth_search is not None:
        cargs.extend(["-depthsearch", str(depth_search)])
    if keep_temp_files:
    ...
    ret = MeasureBbThickOutputs(
        root=execution.output_file("."),
        maxfill=execution.output_file(f"{outdir}/maxfill.nii.gz") if outdir is not None else None,
        bb_thick=execution.output_file(f"{outdir}/bb_thick.nii.gz") if outdir is not None else None,
        bb_thick_smooth=execution.output_file(f"{outdir}/bb_thick_smooth.nii.gz") if outdir is not None else None,
        bb_thick_niml=execution.output_file(f"{outdir}/bb_thick.niml.dset") if outdir is not None else None,
        bb_thick_smooth_niml=execution.output_file(f"{outdir}/bb_thick_smooth.niml.dset") if outdir is not None else None,
        maskset_output=execution.output_file(f"{outdir}/maskset.nii.gz") if outdir is not None else None,
        maskset_resampled=execution.output_file(f"{outdir}/maskset_rs.nii.gz", optional=True) if outdir is not None else None,
        anat_surface=execution.output_file(f"{outdir}/anat.gii") if outdir is not None else None,
        quick_spec=execution.output_file(f"{outdir}/quick.spec") if outdir is not None else None,
    )
    execution.run(cargs)
    return ret
