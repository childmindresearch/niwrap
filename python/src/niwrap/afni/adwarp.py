...
def adwarp(
    ...
    cargs = []
    cargs.append("adwarp")
    cargs.extend(["-apar", execution.input_file(apar)])
    cargs.extend(["-dpar", dpar])
    ...
    if resam is not None:
        cargs.extend(["-resam", resam])
    if thr is not None:
        cargs.extend(["-thr", thr])
    if func is not None:
        cargs.extend(["-func", func])
    ret = AdwarpOutputs(
        root=execution.output_file("."),
        header_output=execution.output_file(f"{prefix}.HEAD", optional=True) if prefix is not None else None,
        brick_output=execution.output_file(f"{prefix}.BRIK", optional=True) if prefix is not None else None,
    )
    execution.run(cargs)
    return ret
