...
def v_3d_anhist(
    ...
    if winsorize is not None:
        cargs.extend(["-w", winsorize])
    if top_2peaks:
        cargs.append("-2")
    if label is not None:
        cargs.extend(["-label", label])
    if filename is not None:
        cargs.extend(["-fname", filename])
    ret = V3dAnhistOutputs(
        root=execution.output_file("."),
        output_1_d=execution.output_file(f"Anhist.1D", optional=True),
        output_ps=execution.output_file(f"Anhist.ps", optional=True),
    )
    execution.run(cargs)
    return ret
