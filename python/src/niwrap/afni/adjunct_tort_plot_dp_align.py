...
def adjunct_tort_plot_dp_align(
    ...
    output_prefix: str,
    enorm_max: float | int | None = None,
    enorm_hline: float | int | None = None,
    no_svg: bool = False,
    ...
    ret = AdjunctTortPlotDpAlignOutputs(
        ...
        align_params=execution.output_file(f"{output_prefix}_align.1D"),
        enorm_file=execution.output_file(f"{output_prefix}_enorm.1D"),
        plot_jpg=execution.output_file(f"{output_prefix}.jpg", optional=True),
        plot_svg=execution.output_file(f"{output_prefix}.svg", optional=True),
    )
    execution.run(cargs)
    return ret
