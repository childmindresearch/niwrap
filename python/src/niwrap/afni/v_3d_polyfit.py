...
def v_3d_polyfit(
    ...
    median_radius: float | int | None = None,
    ...
    if fit_method is not None:
        cargs.extend(["-meth", str(fit_method)])
    ...
    ret = V3dPolyfitOutputs(
        ...
        output_file=execution.output_file(f"{output_prefix}.nii.gz", optional=True) if output_prefix is not None else None,
        resid_file=execution.output_file(f"{resid_prefix}.nii.gz", optional=True) if resid_prefix is not None else None,
        coeff_file=execution.output_file(f"{coeff_output}.1D", optional=True) if coeff_output is not None else None,
    )
    execution.run(cargs)
    return ret
