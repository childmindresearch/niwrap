...
def v_3d_anova2(
    ...
    if fb is not None:
        cargs.extend(["-fb", fb])
    if fab is not None:
        cargs.extend(["-fab", fab])
    if amean is not None:
        cargs.extend(["-amean", amean])
    if bmean is not None:
        cargs.extend(["-bmean", bmean])
    if xmean is not None:
        cargs.extend(["-xmean", xmean])
    if adiff is not None:
        cargs.extend(["-adiff", adiff])
    if bdiff is not None:
        cargs.extend(["-bdiff", bdiff])
    if xdiff is not None:
        cargs.extend(["-xdiff", xdiff])
    if acontr is not None:
        cargs.extend(["-acontr", acontr])
    if bcontr is not None:
        cargs.extend(["-bcontr", bcontr])
    if xcontr is not None:
        cargs.extend(["-xcontr", xcontr])
    ...
    ret = V3dAnova2Outputs(
        ...
        output_ftr=execution.output_file(f"{ftr}.+tlrc", optional=True) if ftr is not None else None,
        output_fa=execution.output_file(f"{fa}.+tlrc", optional=True) if fa is not None else None,
        output_fb=execution.output_file(f"{fb}.+tlrc", optional=True) if fb is not None else None,
        output_fab=execution.output_file(f"{fab}.+tlrc", optional=True) if fab is not None else None,
        output_amean=execution.output_file(f"{amean}.+tlrc", optional=True) if amean is not None else None,
        output_bmean=execution.output_file(f"{bmean}.+tlrc", optional=True) if bmean is not None else None,
        output_xmean=execution.output_file(f"{xmean}.+tlrc", optional=True) if xmean is not None else None,
        output_adiff=execution.output_file(f"{adiff}.+tlrc", optional=True) if adiff is not None else None,
        output_bdiff=execution.output_file(f"{bdiff}.+tlrc", optional=True) if bdiff is not None else None,
        output_xdiff=execution.output_file(f"{xdiff}.+tlrc", optional=True) if xdiff is not None else None,
        output_acontr=execution.output_file(f"{acontr}.+tlrc", optional=True) if acontr is not None else None,
        output_bcontr=execution.output_file(f"{bcontr}.+tlrc", optional=True) if bcontr is not None else None,
        output_xcontr=execution.output_file(f"{xcontr}.+tlrc", optional=True) if xcontr is not None else None,
        output_bucket=execution.output_file(f"{bucket}.+tlrc", optional=True) if bucket is not None else None,
    )
    execution.run(cargs)
    return ret
