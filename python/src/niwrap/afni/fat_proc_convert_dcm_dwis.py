...
def fat_proc_convert_dcm_dwis(
    ...
    origin_xyz: list[float | int] | None = None,
    flip_x: bool = False,
    flip_y: bool = False,
    flip_z: bool = False,
    no_flip: bool = False,
    ...
    if origin_xyz is not None and (len(origin_xyz) != 3): 
        raise ValueError(f"Length of 'origin_xyz' must be 3 but was {len(origin_xyz)}")
    ...
    ret = FatProcConvertDcmDwisOutputs(
        ...
        output_nifti=execution.output_file(f"{output_prefix}.nii.gz"),
        output_rvec=execution.output_file(f"{output_prefix}.rvec"),
        output_bval=execution.output_file(f"{output_prefix}.bval"),
        output_mat_a=execution.output_file(f"{output_prefix}_matA.dat", optional=True),
        output_mat_t=execution.output_file(f"{output_prefix}_matT.dat", optional=True),
        output_cvec=execution.output_file(f"{output_prefix}_cvec.dat", optional=True),
    )
    execution.run(cargs)
    return ret
