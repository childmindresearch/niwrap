...
class Stage:
    ...
    def outputs(
        ...
        ret = StageOutputs(
            ...
            self.convergence=self.convergence.outputs(execution),
        )
        return ret
...
def ants_registration(
    ...
    ret = AntsRegistrationOutputs(
        ...
        initial_moving_transform=initial_moving_transform.outputs(execution),
        stages=[stages.outputs(execution) for stages in stages],
        winsorize_image_intensities=winsorize_image_intensities.outputs(execution),
        masks=masks.outputs(execution),
    )
    execution.run(cargs)
    return ret
