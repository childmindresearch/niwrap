...
def atropos(
    ...
    class), and/or an MRF prior to enforce spatial smoothing of the labels. All
    segmentation images including priors and masks must be in the same voxel and
    physical space. Similar algorithms include FAST and SPM.
    ...
    cargs.extend(["--intensity-image", *intensity_image])
    if bspline is not None:
        cargs.extend(["--bspline", bspline])
    cargs.extend(["--initialization", initialization])
    if partial_volume_label_set is not None:
        cargs.extend(["--partial-volume-label-set", *partial_volume_label_set])
    if use_partial_volume_likelihoods is not None:
        cargs.extend(["--use-partial-volume-likelihoods", str(use_partial_volume_likelihoods)])
    if posterior_formulation is not None:
        cargs.extend(["--posterior-formulation", posterior_formulation])
    cargs.extend(["-x", mask_image])
    cargs.extend(["--convergence", convergence])
    cargs.extend(["--likelihood-model", likelihood_model])
    if mrf is not None:
        cargs.extend(["-m", mrf])
    if icm is not None:
        cargs.extend(["--icm", icm])
    if use_random_seed is not None:
        cargs.extend(["--use-random-seed", str(use_random_seed)])
    cargs.extend(["-o", output])
    if minimize_memory_usage is not None:
        cargs.extend(["--minimize-memory-usage", str(minimize_memory_usage)])
    if winsorize_outliers is not None:
        cargs.extend(["--winsorize-outliers", winsorize_outliers])
    if use_euclidean_distance is not None:
        cargs.extend(["--use-euclidean-distance", str(use_euclidean_distance)])
    if label_propagation is not None:
        cargs.extend(["--label-propagation", label_propagation])
    if verbose is not None:
        cargs.extend(["--verbose", str(verbose)])
    ret = AtroposOutputs(
        root=execution.output_file("."),
        classified_image=execution.output_file(f"[CLASSIFIED_IMAGE]"),
        posterior_probability_images=execution.output_file(f"[POSTERIOR_PROBABILITY_IMAGE_FILE_FORMAT]"),
    )
    execution.run(cargs)
    return ret
