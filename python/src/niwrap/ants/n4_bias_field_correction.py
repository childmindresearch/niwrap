...
class N4BiasFieldCorrectionConvergence:
    ...
    def run(
        ...
        cargs.append(
            "[" +
            "x".join(map(str, self.convergence_)) +
            (("," + str(self.convergence_threshold)) if self.convergence_threshold is not None else "") +
            "]"
        )
        return cargs
...
class N4BiasFieldCorrectionBsplineFitting:
    ...
    Description missing
    """
    spline_distance: list[float | int]
    """Description missing"""
    ...
    def run(
        ...
        cargs.append(
            "[" +
            "x".join(map(str, self.spline_distance)) +
...
class N4BiasFieldCorrectionHistogramSharpening:
    ...
    """Description missing"""
    wiener_noise: float | int | None = None
    """Description missing"""
    ...
    def run(
        ...
        if self.fwhm is not None or self.wiener_noise is not None or self.number_of_histogram_bins is not None:
            cargs.append(
                "[" +
                (str(self.fwhm) if self.fwhm is not None else "") +
                (("," + str(self.wiener_noise)) if self.wiener_noise is not None else "") +
                (("," + str(self.number_of_histogram_bins)) if self.number_of_histogram_bins is not None else "") +
                "]"
            )
        return cargs
...
def n4_bias_field_correction(
    ...
    if convergence is not None:
        cargs.extend(["--convergence", *convergence.run(execution)])
    if bspline_fitting is not None:
        cargs.extend(["--bspline-fitting", *bspline_fitting.run(execution)])
    if histogram_sharpening is not None:
        cargs.extend(["--histogram-sharpening", *histogram_sharpening.run(execution)])
