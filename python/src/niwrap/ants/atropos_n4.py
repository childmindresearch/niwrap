...
class AntsAtroposN4ShSegmentationPriors:
    ...
    segmentation_priors_folder: InputPathType | None = "."
    """Included so."""
    ...
    def run(
        ...
        if self.segmentation_priors_pattern is not None or self.segmentation_priors_folder is not None:
            cargs.append(
                (self.segmentation_priors_pattern if self.segmentation_priors_pattern is not None else "") +
                "/" +
                (execution.input_file(self.segmentation_priors_folder) if self.segmentation_priors_folder is not None else "")
            )
        return cargs
...
def ants_atropos_n4_sh(
    ...
    atropos_segmentation_prior_weight: float | int | None = None,
    ...
    cargs.extend(["-a", execution.input_file(input_image)])
    cargs.extend(["-x", execution.input_file(mask_image)])
    cargs.extend(["-c", str(number_of_classes)])
    cargs.extend(["-o", output_prefix])
    if max_n4_atropos_iterations is not None:
        cargs.extend(["-m", str(max_n4_atropos_iterations)])
    if max_atropos_iterations is not None:
        cargs.extend(["-n", str(max_atropos_iterations)])
    cargs.extend(["-p", *segmentation_priors.run(execution)])
    if mrf is not None:
        cargs.extend(["-r", mrf])
    if denoise_anatomical_images is not None:
        cargs.extend(["-g", str(denoise_anatomical_images)])
    if posterior_formulation is not None:
        cargs.extend(["-b", posterior_formulation])
    if label_propagation is not None:
        cargs.extend(["-l", label_propagation])
    if posterior_label_for_n4_weight_mask is not None:
        cargs.extend(["-y", posterior_label_for_n4_weight_mask])
    if image_file_suffix is not None:
        cargs.extend(["-s", image_file_suffix])
    if keep_temporary_files is not None:
        cargs.extend(["-k", str(keep_temporary_files)])
    if use_random_seeding is not None:
        cargs.extend(["-u", str(use_random_seeding)])
    if atropos_segmentation_prior_weight is not None:
        cargs.extend(["-w", str(atropos_segmentation_prior_weight)])
    if n4_convergence is not None:
        cargs.extend(["-e", n4_convergence])
    if n4_shrink_factor is not None:
        cargs.extend(["-f", str(n4_shrink_factor)])
    if n4_bspline_params is not None:
        cargs.extend(["-q", n4_bspline_params])
    if atropos_segmentation_icm is not None:
        cargs.extend(["-i", atropos_segmentation_icm])
    if atropos_segmentation_use_euclidean_distance is not None:
        cargs.extend(["-j", str(atropos_segmentation_use_euclidean_distance)])
    if test_debug_mode is not None:
        cargs.extend(["-z", str(test_debug_mode)])
    ret = AntsAtroposN4ShOutputs(
        root=execution.output_file("."),
        n4_corrected=execution.output_file(f"{output_prefix}N4Corrected.[OUTPUT_SUFFIX]"),
        segmentation=execution.output_file(f"{output_prefix}Segmentation.[OUTPUT_SUFFIX]"),
        segmentation_posteriors=execution.output_file(f"{output_prefix}SegmentationPosteriors.[OUTPUT_SUFFIX]"),
    )
    execution.run(cargs)
    return ret
