...
def scene_capture_image(
    ...
    ret = SceneCaptureImageOutputs(
        root=execution.output_file("."),
        size_width_height=size_width_height.outputs(execution),
        resolution=resolution.outputs(execution),
        set_map_yoke=set_map_yoke.outputs(execution),
        conn_db_login=conn_db_login.outputs(execution),
    )
    execution.run(cargs)
    return ret
