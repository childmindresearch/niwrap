...
def surface_average(
    ...
    ret = SurfaceAverageOutputs(
        ...
        stddev_metric_out=execution.output_file(f"{pathlib.Path(stddev_metric_out).stem}"),
        uncert_metric_out=execution.output_file(f"{pathlib.Path(uncert_metric_out).stem}"),
    )
    execution.run(cargs)
    return ret
