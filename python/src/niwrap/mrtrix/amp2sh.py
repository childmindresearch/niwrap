...
def amp2sh(
    ...
    More information:
    https://mrtrix.readthedocs.io/en/latest/reference/commands/amp2sh.html
    ...
    if rician is not None:
        cargs.extend(["-rician", execution.input_file(rician)])
    if grad is not None:
    ...
    cargs.extend(["", execution.input_file(amp)])
    cargs.extend(["", execution.input_file(sh)])
    ret = Amp2shOutputs(
        root=execution.output_file("."),
        sh=execution.output_file(f"{pathlib.Path(sh).stem}"),
    )
    execution.run(cargs)
    return ret
