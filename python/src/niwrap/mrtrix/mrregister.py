...
def mrregister(
    ...
    rigid_scale: list[float | int] = None,
    rigid_niter: list[int] = None,
    ...
    affine_scale: list[float | int] = None,
    affine_niter: list[int] = None,
    ...
    init_rotation_search_angles: list[float | int] = None,
    init_rotation_search_scale: float | int | None = None,
    ...
    linstage_iterations: list[int] = None,
    linstage_optimiser_first: typing.Literal["algorithm"] | None = None,
    ...
    nl_update_smooth: float | int | None = None,
    nl_disp_smooth: float | int | None = None,
    nl_grad_step: float | int | None = None,
    nl_lmax: list[int] = None,
    diagnostics_image: str | None = None,
    directions: InputPathType | None = None,
    noreorientation: bool = False,
    mc_weights: list[float | int] = None,
    ...
    """
    mrregister by David Raffelt (david.raffelt@florey.edu.au) & Max Pietsch
    ...
    * If FOD registration is being performed:
    ...
    Args:
        ...
        transformed_midway: image1 and image2 after registration transformed and
            regridded to the midway space. Note that -transformed_midway needs to be
            repeated for each contrast if multi-contrast registration is used.
        ...
        rigid_1tomidway: the output text file containing the rigid
            transformation that aligns image1 to image2 in their common midway space
            as a 4x4 matrix
        rigid_2tomidway: the output text file containing the rigid
            transformation that aligns image2 to image1 in their common midway space
            as a 4x4 matrix
        rigid_init_translation: initialise the translation and centre of
            rotation
            Valid choices are:
            mass (aligns the centers of mass of both
            images, default),
            geometric (aligns geometric image centres)
            and none.
        rigid_init_rotation: initialise the rotation Valid choices are:
            search (search for the best rotation using mean
            squared residuals),
            moments (rotation based on directions of
            intensity variance with respect to centre of
            mass),
            none (default).
        ...
        affine_init_translation: initialise the translation and centre of
            ...
            images),
            geometric (aligns geometric image centres)
            and none. (Default: mass)
        affine_init_rotation: initialise the rotation Valid choices are:
            search (search for the best rotation using
        ...
        affine_lmax: explicitly set the lmax to be used per scale factor in
            affine FOD registration. By default FOD registration will use lmax 0,2,4
        ...
        linstage_optimiser_last: Cost function optimisation algorithm to use at
            ...
            choices: bbgd (Barzilai-Borwein gradient descent) or gd (simple gradient
        ...
        nl_warp_full: output all warps used during registration. This saves four
            different warps that map each image to a midway space and their inverses
            in a single 5D image file. The 4th image dimension indexes the x,y,z
            component of the deformation vector and the 5th dimension indexes the
            field in this order: image1->midway, midway->image1, image2->midway,
            midway->image2. Where image1->midway defines the field that maps image1
            onto the midway space using the reverse convention When linear
            registration is performed first, the estimated linear transform will be
            included in the comments of the image header, and therefore the entire
            linear and non-linear transform can be applied (in either direction)
        ...
        nl_niter: the maximum number of iterations. This can be specified either
            as a single number for all multi-resolution levels, or a single value
            for each level. (Default: 50)
        ...
        nl_lmax: explicitly set the lmax to be used per scale factor in
            ...
            0,2,4 with default scale factors 0.25,0.5,1.0 respectively. Note that no
            reorientation will be performed with lmax = 0.
        ...
        noreorientation: turn off FOD reorientation. Reorientation is on by
            default if the number of volumes in the 4th dimension corresponds to the
            number of coefficients in an antipodally symmetric spherical harmonic
            series (i.e. 6, 15, 28, 45, 66 etc)
    ...
    if rigid_init_rotation is not None:
        cargs.extend(["-rigid_init_rotation", rigid_init_rotation])
    if rigid_init_matrix is not None:
        cargs.extend(["-rigid_init_matrix", execution.input_file(rigid_init_matrix)])
    if rigid_scale is not None:
        cargs.extend(["-rigid_scale", *map(str, rigid_scale)])
    if rigid_niter is not None:
        cargs.extend(["-rigid_niter", *map(str, rigid_niter)])
    if rigid_metric is not None:
        cargs.extend(["-rigid_metric", rigid_metric])
    if rigid_metric_diff_estimator is not None:
        cargs.extend(["-rigid_metric.diff.estimator", rigid_metric_diff_estimator])
    ...
    if affine_init_rotation is not None:
        cargs.extend(["-affine_init_rotation", affine_init_rotation])
    if affine_init_matrix is not None:
        cargs.extend(["-affine_init_matrix", execution.input_file(affine_init_matrix)])
    if affine_scale is not None:
        cargs.extend(["-affine_scale", *map(str, affine_scale)])
    if affine_niter is not None:
        cargs.extend(["-affine_niter", *map(str, affine_niter)])
    if affine_metric is not None:
        cargs.extend(["-affine_metric", affine_metric])
    if affine_metric_diff_estimator is not None:
        cargs.extend(["-affine_metric.diff.estimator", affine_metric_diff_estimator])
    ...
    if init_rotation_search_angles is not None:
        cargs.extend(["-init_rotation.search.angles", *map(str, init_rotation_search_angles)])
    if init_rotation_search_scale is not None:
        cargs.extend(["-init_rotation.search.scale", str(init_rotation_search_scale)])
    if init_rotation_search_directions is not None:
        cargs.extend(["-init_rotation.search.directions", str(init_rotation_search_directions)])
    ...
    if init_rotation_search_global_iterations is not None:
        cargs.extend(["-init_rotation.search.global.iterations", str(init_rotation_search_global_iterations)])
    if linstage_iterations is not None:
        cargs.extend(["-linstage.iterations", *map(str, linstage_iterations)])
    if linstage_optimiser_first is not None:
        cargs.extend(["-linstage.optimiser.first", linstage_optimiser_first])
    if linstage_optimiser_last is not None:
        cargs.extend(["-linstage.optimiser.last", linstage_optimiser_last])
    if linstage_optimiser_default is not None:
        cargs.extend(["-linstage.optimiser.default", linstage_optimiser_default])
    ...
    if nl_scale is not None:
        cargs.extend(["-nl_scale", *map(str, nl_scale)])
    if nl_niter is not None:
        cargs.extend(["-nl_niter", *map(str, nl_niter)])
    if nl_update_smooth is not None:
        cargs.extend(["-nl_update_smooth", str(nl_update_smooth)])
    if nl_disp_smooth is not None:
        cargs.extend(["-nl_disp_smooth", str(nl_disp_smooth)])
    if nl_grad_step is not None:
        cargs.extend(["-nl_grad_step", str(nl_grad_step)])
    if nl_lmax is not None:
        cargs.extend(["-nl_lmax", *map(str, nl_lmax)])
    if diagnostics_image is not None:
        cargs.extend(["-diagnostics_image", diagnostics_image])
    if directions is not None:
        cargs.extend(["-directions", execution.input_file(directions)])
    if noreorientation:
        cargs.append("-noreorientation")
    if mc_weights is not None:
        cargs.extend(["-mc_weights", *map(str, mc_weights)])
    ...
    ret = MrregisterOutputs(
        ...
        rigid_1tomidway=execution.output_file(f"{pathlib.Path(rigid_1tomidway).stem}") if rigid_1tomidway is not None else None,
        rigid_2tomidway=execution.output_file(f"{pathlib.Path(rigid_2tomidway).stem}") if rigid_2tomidway is not None else None,
        rigid_log=execution.output_file(f"{pathlib.Path(rigid_log).stem}") if rigid_log is not None else None,
        affine=execution.output_file(f"{pathlib.Path(affine).stem}") if affine is not None else None,
        affine_1tomidway=execution.output_file(f"{pathlib.Path(affine_1tomidway).stem}") if affine_1tomidway is not None else None,
        affine_2tomidway=execution.output_file(f"{pathlib.Path(affine_2tomidway).stem}") if affine_2tomidway is not None else None,
        affine_log=execution.output_file(f"{pathlib.Path(affine_log).stem}") if affine_log is not None else None,
        nl_warp_full=execution.output_file(f"{pathlib.Path(nl_warp_full).stem}") if nl_warp_full is not None else None,
    )
    execution.run(cargs)
    return ret
