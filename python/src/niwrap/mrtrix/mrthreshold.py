...
MRTHRESHOLD_METADATA = Metadata(
    id="5d0e94e5df07360da1d3d993bd87a2e1efc39693",
    name="mrthreshold",
...
def mrthreshold(
    ...
    * If not using any explicit thresholding mechanism:
    ...
    if abs_ is not None:
        cargs.extend(["-abs", str(abs_)])
    if percentile is not None:
        cargs.extend(["-percentile", str(percentile)])
    if top is not None:
        cargs.extend(["-top", str(top)])
    if bottom is not None:
        cargs.extend(["-bottom", str(bottom)])
    if allvolumes:
    ...
    if comparison is not None:
        cargs.extend(["-comparison", comparison])
    if invert:
        cargs.append("-invert")
    if out_masked:
