...
class TckmapConfigOutputs(typing.NamedTuple):
    """
    Output object returned when calling `TckmapConfig.run(...)`.
    """
    root: OutputPathType
...
class TckmapConfig:
    ...
    def outputs(
        ...
        Args:
            ...
            execution: The execution object.
        Returns:
            NamedTuple of outputs (described in `TckmapConfigOutputs`).
        """
        ret = TckmapConfigOutputs(
...
class TckmapOutputs(typing.NamedTuple):
    ...
    config: typing.List[TckmapConfigOutputs]
    """Subcommand outputs"""
...
def tckmap(
    ...
    * If using -precise mapping option:
    ...
    if dixel is not None:
        cargs.extend(["-dixel", dixel])
    if tod is not None:
        cargs.extend(["-tod", str(tod)])
    if contrast is not None:
        cargs.extend(["-contrast", contrast])
    if image is not None:
        cargs.extend(["-image", execution.input_file(image)])
    if vector_file is not None:
        cargs.extend(["-vector_file", execution.input_file(vector_file)])
    ...
    if fwhm_tck is not None:
        cargs.extend(["-fwhm_tck", str(fwhm_tck)])
    if map_zero:
