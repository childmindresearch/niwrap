...
V_5TTGEN_METADATA = Metadata(
    id="e1c8721cf951c39ed38887df51b196592e09eda2",
    name="5ttgen",
...
class V5ttgenFreesurferOutputs(typing.NamedTuple):
    """
    Output object returned when calling `V5ttgenFreesurfer.run(...)`.
    """
    root: OutputPathType
...
class V5ttgenFreesurfer:
    ...
    def run(
        ...
        if self.lut is not None:
            cargs.extend(["-lut", execution.input_file(self.lut)])
        return cargs
    ...
    def outputs(
        ...
        ret = V5ttgenFreesurferOutputs(
            ...
            output=execution.output_file(f"{pathlib.Path(self.output).name}"),
        )
        return ret
...
class V5ttgenFslOutputs(typing.NamedTuple):
    """
    Output object returned when calling `V5ttgenFsl.run(...)`.
    """
    root: OutputPathType
    """Output root folder. This is the root folder for all outputs."""
...
class V5ttgenFsl:
    ...
    def run(
        ...
        if self.t2 is not None:
            cargs.extend(["-t2", execution.input_file(self.t2)])
        if self.mask is not None:
            cargs.extend(["-mask", execution.input_file(self.mask)])
        if self.premasked:
    ...
    def outputs(
        ...
        ret = V5ttgenFslOutputs(
            root=execution.output_file("."),
            output_=execution.output_file(f"{pathlib.Path(self.output).name}"),
        )
        return ret
...
class V5ttgenGifOutputs(typing.NamedTuple):
    """
    Output object returned when calling `V5ttgenGif.run(...)`.
    """
    root: OutputPathType
    """Output root folder. This is the root folder for all outputs."""
...
class V5ttgenGif:
    ...
    def outputs(
        ...
        ret = V5ttgenGifOutputs(
            root=execution.output_file("."),
            output_2=execution.output_file(f"{pathlib.Path(self.output).name}"),
        )
        return ret
...
class V5ttgenHsvsOutputs(typing.NamedTuple):
    """
    Output object returned when calling `V5ttgenHsvs.run(...)`.
    """
    root: OutputPathType
    """Output root folder. This is the root folder for all outputs."""
...
class V5ttgenHsvs:
    ...
    def run(
        ...
        if self.template is not None:
            cargs.extend(["-template", execution.input_file(self.template)])
    ...
    def outputs(
        ...
        ret = V5ttgenHsvsOutputs(
            root=execution.output_file("."),
            output_3=execution.output_file(f"{pathlib.Path(self.output).name}"),
        )
        return ret
...
def v_5ttgen(
    ...
    More information:
    https://mrtrix.readthedocs.io/en/3.0.4/reference/commands/5ttgen.html
