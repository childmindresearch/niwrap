...
def tensor2metric(
    ...
    cargs.extend(["", execution.input_file(tensor)])
    ret = Tensor2metricOutputs(
        root=execution.output_file("."),
        adc=execution.output_file(f"{pathlib.Path(adc).stem}") if adc is not None else None,
        fa=execution.output_file(f"{pathlib.Path(fa).stem}") if fa is not None else None,
        ad=execution.output_file(f"{pathlib.Path(ad).stem}") if ad is not None else None,
        rd=execution.output_file(f"{pathlib.Path(rd).stem}") if rd is not None else None,
        cl=execution.output_file(f"{pathlib.Path(cl).stem}") if cl is not None else None,
        cp=execution.output_file(f"{pathlib.Path(cp).stem}") if cp is not None else None,
        cs=execution.output_file(f"{pathlib.Path(cs).stem}") if cs is not None else None,
        value=execution.output_file(f"{pathlib.Path(value).stem}") if value is not None else None,
        vector=execution.output_file(f"{pathlib.Path(vector).stem}") if vector is not None else None,
    )
    execution.run(cargs)
    return ret
