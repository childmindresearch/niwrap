...
def dwibiascorrect(
    ...
    fslgrad_bvecs: list[InputPathType] = None,
    ...
    """
    dwibiascorrect by Robert E. Smith.
    ...
    if fslgrad_bvecs is not None:
        cargs.extend(["-fslgrad", *[execution.input_file(f) for f in fslgrad_bvecs]])
    if mask_image is not None:
        cargs.extend(["-mask", execution.input_file(mask_image)])
    if bias_image is not None:
        cargs.extend(["-bias", execution.input_file(bias_image)])
    if nocleanup:
        cargs.append("-nocleanup")
    if scratch_dir is not None:
        cargs.extend(["-scratch", execution.input_file(scratch_dir)])
    if continue_scratch_dir is not None:
        cargs.extend(["-continue", *[execution.input_file(f) for f in continue_scratch_dir]])
    if info:
        cargs.append("-info")
    if quiet:
        cargs.append("-quiet")
    ...
    if config is not None:
        cargs.extend(["-config", *config])
    if help_:
        cargs.append("-help")
    if version:
        cargs.append("-version")
    if ants_b is not None:
        cargs.extend(["-ants.b", ants_b])
    if ants_c is not None:
        cargs.extend(["-ants.c", ants_c])
