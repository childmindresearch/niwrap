...
TCKSIFT_METADATA = Metadata(
    id="a9c1e857ac82fd3c32668ef8b36d811f7579e28b",
    name="tcksift",
...
def tcksift(
    ...
    Args:
        ...
        term_ratio: termination ratio - defined as the ratio between reduction
            ...
            Smaller values result in more streamlines being filtered
            out.
    ...
    if term_ratio is not None:
        cargs.extend(["-term_ratio", str(term_ratio)])
    if term_mu is not None:
        cargs.extend(["-term_mu", str(term_mu)])
    if info:
        cargs.append("-info")
    if quiet:
        cargs.append("-quiet")
    ...
    ret = TcksiftOutputs(
        ...
        out_selection=execution.output_file(f"{pathlib.Path(out_selection).stem}") if out_selection is not None else None,
    )
    execution.run(cargs)
    return ret
