...
def tcksift2(
    ...
    min_factor: float | int | None = None,
    min_coeff: float | int | None = None,
    max_factor: float | int | None = None,
    max_coeff: float | int | None = None,
    max_coeff_step: float | int | None = None,
    min_cf_decrease: float | int | None = None,
    linear: bool = False,
    info: bool = False,
    quiet: bool = False,
    debug: bool = False,
    ...
    * If using the -linear option:
    ...
    Args:
        ...
        fd_scale_gm: provide this option (in conjunction with -act) to
            heuristically downsize the fibre density estimates based on the presence
            of GM in the voxel. This can assist in reducing tissue interface effects
        ...
        out_mu: output the final value of SIFT proportionality coefficient mu to
            a text file
        ...
        min_coeff: minimum weighting coefficient for an individual streamline;
            ...
            factor = e^(coeff). Note that the -min_factor and -min_coeff options are
            mutually exclusive - you can only provide one. (default: -inf)
        ...
        max_coeff: maximum weighting coefficient for an individual streamline;
            ...
            coefficient basis of the SIFT2 model; these parameters are related as:
            factor = e^(coeff). Note that the -max_factor and -max_coeff options are
            mutually exclusive - you can only provide one. (default: inf)
    ...
    if reg_tv is not None:
        cargs.extend(["-reg_tv", str(reg_tv)])
    if min_td_frac is not None:
        cargs.extend(["-min_td_frac", str(min_td_frac)])
    if min_iters is not None:
        cargs.extend(["-min_iters", str(min_iters)])
    if max_iters is not None:
        cargs.extend(["-max_iters", str(max_iters)])
    if min_factor is not None:
        cargs.extend(["-min_factor", str(min_factor)])
    if min_coeff is not None:
        cargs.extend(["-min_coeff", str(min_coeff)])
    if max_factor is not None:
        cargs.extend(["-max_factor", str(max_factor)])
    if max_coeff is not None:
        cargs.extend(["-max_coeff", str(max_coeff)])
    if max_coeff_step is not None:
        cargs.extend(["-max_coeff_step", str(max_coeff_step)])
    if min_cf_decrease is not None:
        cargs.extend(["-min_cf_decrease", str(min_cf_decrease)])
    if linear:
        cargs.append("-linear")
    if info:
        cargs.append("-info")
    ...
    ret = Tcksift2Outputs(
        ...
        csv_=execution.output_file(f"{pathlib.Path(csv_).stem}") if csv_ is not None else None,
        out_mu=execution.output_file(f"{pathlib.Path(out_mu).stem}") if out_mu is not None else None,
        out_coeffs=execution.output_file(f"{pathlib.Path(out_coeffs).stem}") if out_coeffs is not None else None,
    )
    execution.run(cargs)
    return ret
