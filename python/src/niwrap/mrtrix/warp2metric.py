...
WARP2METRIC_METADATA = Metadata(
    id="5a4db60d2e47d86a13d92aac866afde1edbb7031",
    name="warp2metric",
...
def warp2metric(
    ...
    if fc is not None:
        cargs.extend(["-fc", *fc.run(execution)])
    if jmat is not None:
    ...
    ret = Warp2metricOutputs(
        ...
        jdet=execution.output_file(f"{pathlib.Path(jdet).stem}") if jdet is not None else None,
    )
    execution.run(cargs)
    return ret
