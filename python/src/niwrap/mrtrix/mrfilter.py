...
MRFILTER_METADATA = Metadata(
    id="1d295348edef4c2835da068783b86fe315ffc25e",
    name="mrfilter",
...
def mrfilter(
    ...
    stdev: list[float | int] = None,
    magnitude_: bool = False,
    scanner: bool = False,
    extent: list[int] = None,
    extent_: list[int] = None,
    stdev_: list[float | int] = None,
    fwhm: list[float | int] = None,
    extent_2: list[int] = None,
    zupper: float | int | None = None,
    ...
    """
    mrfilter by Robert E. Smith (robert.smith@florey.edu.au), David Raffelt
    ...
    Args:
        ...
        stdev: the standard deviation of the Gaussian kernel used to smooth the
            ...
            gradients caused by noise. Use this option to override the default stdev
            of 1 voxel. This can be specified either as a single value to be used
            for all 3 axes, or as a comma-separated list of 3 values, one for each
            axis.
        magnitude_: output the gradient magnitude, rather than the default x,y,z
            components
        scanner: define the gradient with respect to the scanner coordinate
            frame of reference.
        extent: specify extent of median filtering neighbourhood in voxels. This
            can be specified either as a single value to be used for all 3 axes, or
            as a comma-separated list of 3 values, one for each axis (default:
            3x3x3).
    ...
    if zlower is not None:
        cargs.extend(["-zlower", str(zlower)])
    if bridge is not None:
        cargs.extend(["-bridge", str(bridge)])
    ...
    ret = MrfilterOutputs(
        ...
        maskout=execution.output_file(f"{pathlib.Path(maskout).stem}") if maskout is not None else None,
    )
    execution.run(cargs)
    return ret
