...
def tckglobal(
    ...
    ppot: float | int | None = None,
    cpot: float | int | None = None,
    ...
    if length is not None:
        cargs.extend(["-length", str(length)])
    if weight is not None:
        cargs.extend(["-weight", str(weight)])
    if ppot is not None:
        cargs.extend(["-ppot", str(ppot)])
    if cpot is not None:
        cargs.extend(["-cpot", str(cpot)])
    if t0 is not None:
        cargs.extend(["-t0", str(t0)])
    if t1 is not None:
    ...
    if density is not None:
        cargs.extend(["-density", str(density)])
    if prob is not None:
        cargs.extend(["-prob", *map(str, prob)])
    if beta is not None:
        cargs.extend(["-beta", str(beta)])
    ...
    ret = TckglobalOutputs(
        ...
        fod=execution.output_file(f"{pathlib.Path(fod).stem}") if fod is not None else None,
        fiso=execution.output_file(f"{pathlib.Path(fiso).stem}") if fiso is not None else None,
        eext=execution.output_file(f"{pathlib.Path(eext).stem}") if eext is not None else None,
        etrend=execution.output_file(f"{pathlib.Path(etrend).stem}") if etrend is not None else None,
    )
    execution.run(cargs)
    return ret
